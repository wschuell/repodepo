...
testdb = rd.repo_database.Database(
    db_name="testdb_repotools", db_type="sqlite", data_folder="dummy_clones"
)
testdb.init_db()
...
testdb.add_filler(
    generic.PackageFiller(
        ...
        data_folder=os.path.join(
            os.path.dirname(os.path.dirname(rd.__file__)),
            "tests",
            "testmodule",
            "dummy_data",
        ),
...
testdb.add_filler(
    github_rest.GHLoginsFiller(fail_on_wait=True, workers=workers)
)  # Disambiguating emails by associating them to their GH logins.
