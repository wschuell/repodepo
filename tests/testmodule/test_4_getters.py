...
subclasses_Pgetters = inspect.getmembers(
    project_getters,
    lambda elt: (
        inspect.isclass(elt)
        and issubclass(elt, project_getters.ProjectGetter)
        and elt != project_getters.ProjectGetter
    ),
)
...
subclasses_Ugetters = inspect.getmembers(
    user_getters,
    lambda elt: (
        inspect.isclass(elt)
        and issubclass(elt, user_getters.UserGetter)
        and elt != user_getters.UserGetter
    ),
)
...
subclasses_combined_g = inspect.getmembers(
    combined_getters,
    lambda elt: (
        inspect.isclass(elt)
        and issubclass(elt, combined_getters.CombinedGetter)
        and elt != combined_getters.CombinedGetter
    ),
)
...
def sdb(request):
    orig_db = repodepo.repo_database.Database(
        db_name="travis_ci_test_repo_tools",
        db_type="postgres",
        data_folder=os.path.join(os.path.dirname(__file__), "dummy_data"),
    )
    ...
    if os.path.exists(filename):
        os.remove(filename)
    db = repodepo.repo_database.Database(
        db_name="travis_ci_test_repo_tools_comparison",
        db_type=request.param,
        data_folder=os.path.join(os.path.dirname(__file__), "dummy_data"),
    )
...
def test_gettersP(testdb, time_window, Pgetter, aggregated, cumulative):
    testdb.init_db()
    df = Pgetter().get_result(
        db=testdb, aggregated=aggregated, time_window=time_window, cumulative=cumulative
    )


def test_gettersUuser(testdb, time_window, Ugetter, identity_id, cumulative):
    testdb.init_db()
    df = Ugetter().get_result(
        db=testdb,
        aggregated=True,
        time_window=time_window,
        cumulative=cumulative,
...
def test_value_gettersP(pdb, sdb, time_window, Pgetter, aggregated, cumulative):
    ...
    sdb.init_db()
    p = Pgetter().get_result(
        db=pdb, aggregated=aggregated, time_window=time_window, cumulative=cumulative
    )
    s = Pgetter().get_result(
        db=sdb, aggregated=aggregated, time_window=time_window, cumulative=cumulative
    )
    assert p.equals(s)
...
def test_value_edge_getters(pdb, sdb):
    ...
    assert (
        p.nonzero()[0].shape == s.nonzero()[0].shape
        and p.nonzero()[1].shape == s.nonzero()[1].shape
        and (p.nonzero()[0] == s.nonzero()[0]).all()
        and (p.nonzero()[1] == s.nonzero()[1]).all()
    ), "Different sparsity structure"
    assert (np.abs(p.data - s.data) * 2.0 / (p.data + s.data) <= epsilon).all()
...
def test_value_edge_cr(pdb, sdb):
    p = edge_getters.DevToRepoAddDailyCommits(
        db=pdb, repo_list=list(range(5)), daily_commits=0.005 / 7.0
    ).get_result()
    s = edge_getters.DevToRepoAddDailyCommits(
        db=sdb, repo_list=list(range(5)), daily_commits=0.005 / 7.0
    ).get_result()
