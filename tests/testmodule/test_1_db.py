...
def testdb(request):
    db = repodepo.repo_database.Database(
        db_name="travis_ci_test_repo_tools", db_type=request.param
    )
    db.clean_db()
    db.init_db()
    yield db
...
def otherdb(request):
    db = repodepo.repo_database.Database(
        db_name="travis_ci_test_repo_tools_dumptest", db_type=request.param
    )
    db.clean_db()
    db.init_db()
    yield db
...
def test_createdb(dbtype):
    db = repodepo.repo_database.Database(
        db_name="travis_ci_test_repo_tools", db_type=dbtype, clean_first=True
    )
...
def test_dl(testdb):
    ...
    testdb.register_url(source="GitHub", repo_url="https://github.com/test/test")
    testdb.register_repo(source="GitHub", repo="test", owner="test")
    testdb.submit_download_attempt(
        source="GitHub", owner="test", repo="test", success=False
    )
    testdb.submit_download_attempt(
        source="GitHub", owner="test", repo="test", success=True
    )
