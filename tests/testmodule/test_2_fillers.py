...
def test_sources(testdb):
    testdb.add_filler(
        generic.SourcesFiller(source="GitHub", source_urlroot="github.com")
    )
    testdb.fill_db()
...
def test_urls(testdb):
    ...
    testdb.add_filler(
        generic.URLFiller(
            url_list=["github.com/deepcharles/ruptures"],
            data_folder=os.path.join(os.path.dirname(__file__), "dummy_data"),
        )
    )
    testdb.fill_db()
...
def test_sources3(testdb):
    testdb.add_filler(
        generic.SourcesFiller(
            source=["GitHub", "blah"], source_urlroot=["github.com", None]
        )
    )
    testdb.fill_db()
...
def test_identities(testdb):
    testdb.add_filler(
        generic.IdentitiesFiller(
            identity_type="test_identities",
            identities_list_file="identities.csv",
            data_folder=os.path.join(os.path.dirname(__file__), "dummy_data"),
        )
    )
    testdb.fill_db()
...
def test_identities2(testdb):
    testdb.add_filler(
        generic.IdentitiesFiller(
            identity_type="test_identities",
            identities_list_file="identities_2.csv",
            data_folder=os.path.join(os.path.dirname(__file__), "dummy_data"),
        )
    )
    testdb.fill_db()
...
def test_identities3(testdb):
    testdb.add_filler(
        generic.IdentitiesFiller(
            identity_type="test_identities",
            identities_list=[
                ("blah", {"name": "blih", "age": 25}),
                ("bleh", '{"name":"bloh","age":35}'),
            ],
            data_folder=os.path.join(os.path.dirname(__file__), "dummy_data"),
        )
    )
...
def test_botlist(testdb):
    ...
    testdb.add_filler(
        bot_fillers.BotListFiller(bot_list=["blah['bot']"], identity_type="dummy_data")
    )
    testdb.fill_db()
...
def test_botfile(testdb):
    ...
    testdb.add_filler(
        bot_fillers.BotFileFiller(
            bot_file="botlist.csv",
            identity_type="test_identities",
            data_folder=os.path.join(os.path.dirname(__file__), "dummy_data"),
        )
    )
...
def test_commits(testdb, workers_count):
    ...
    testdb.add_filler(
        commit_info.CommitsFiller(data_folder="dummy_clones", workers=workers_count)
    )
    testdb.fill_db()
...
def test_github(testdb):
    ...
    testdb.add_filler(generic.RepositoriesFiller())
    testdb.add_filler(
        github_rest.ForksFiller(fail_on_wait=True, workers=workers, no_unauth=True)
    )
    testdb.add_filler(
    ...
    testdb.add_filler(
        github_rest.GHLoginsFiller(fail_on_wait=True, workers=workers, no_unauth=True)
    )
    testdb.add_filler(
        github_rest.StarsFiller(fail_on_wait=True, workers=workers, no_unauth=True)
    )
    testdb.add_filler(
        github_rest.FollowersFiller(fail_on_wait=True, workers=workers, no_unauth=True)
    )
    testdb.fill_db()
...
def test_reset_merged_identities(testdb):
    ...
    assert (
        testdb.count_users() == testdb.count_identities()
    ), "There should be as many users as identities"
    testdb.fillers = []
    testdb.add_filler(
        github_rest.GHLoginsFiller(
            fail_on_wait=True, workers=workers, force=True, no_unauth=True
        )
    )
    testdb.fill_db()
...
def test_filters(testdb):
    ...
    testdb.add_filler(
        deps_filters_fillers.PackageDepsFilter(
            input_list=["crates/blah", ("crites", "bloh"), "blyh"]
        )
    )
    testdb.add_filler(
        deps_filters_fillers.RepoDepsFilter(
            input_list=["GitHub/blah/blih", ("GitHub", "bloh", "bluh"), "blyh/bluh"]
        )
    )
    testdb.add_filler(
        deps_filters_fillers.RepoEdgesDepsFilter(
            input_list=[
                ("GitHub/blah/blih", "Gitlab/blih/blah"),
                ("GitHub", "bloh", "bluh", "GitHub", "blyh", "bloh"),
            ]
        )
    )
    testdb.add_filler(
        deps_filters_fillers.PackageEdgesDepsFilter(
            input_list=[
                ("crates/bloh", "juliahub/blah"),
                ("crates", "bloh", "crates", "blyh"),
            ]
        )
    )
    testdb.fill_db()
