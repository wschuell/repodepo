...
def test_github_gql(testdb):
	...
	testdb.add_filler(generic.PackageFiller(package_list_file='packages.csv',data_folder=os.path.join(os.path.dirname(os.path.abspath(__file__)),'dummy_data')))
	...
	testdb.add_filler(github_gql.CommitterRandomCommitLoginsGQLFiller(fail_on_wait=True,workers=workers))
	...
	testdb.add_filler(github_gql.CommitCommentsGQLFiller(fail_on_wait=True,workers=workers,secondary_page_size=1))
	...
	testdb.add_filler(github_gql.UserInfoGQLFiller(fail_on_wait=True,workers=workers,additional_data={'hireable':'isHireable','email':'email','gh_id':'id'}))
...
def test_complete_issues_pr(testdb):
	...
	testdb.add_filler(github_gql.CompleteIssuesGQLFiller(fail_on_wait=True,workers=workers,secondary_page_size=1))
	testdb.add_filler(github_gql.CompletePullRequestsGQLFiller(fail_on_wait=True,workers=workers,secondary_page_size=1))
