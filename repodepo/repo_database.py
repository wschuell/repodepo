...
class Database(object):
    ...
    def __init__(
        ...
        if db_type == "sqlite":
            if db_name.startswith(":memory:"):
                ...
                self.connection = sqlite3.connect(
                    self.db_path, timeout=timeout, detect_types=sqlite3.PARSE_DECLTYPES
                )
                ...
                if 'database "{}" does not exist\n'.format(db_name) in str(e):
                    ...
                    self.logger.warning(
                        "Database {} does not exist: trying to create it via connecting primarily to database {}".format(
                            self.db_conninfo["database"], conninfo_nodb["database"]
                        )
                    )
        ...
        if computation_db_name.startswith(":") or os.path.isabs(computation_db_name):
            ...
            self.computation_db_name = os.path.join(
                self.data_folder, computation_db_name
            )
        ...
        with open(
            os.path.join(os.path.dirname(__file__), f"initscript_{self.db_type}.sql"),
            "r",
        ) as f:
            self.DB_INIT = f.read()
    ...
    def check_structure(self):
        ...
        if len(ans) == 0:
            raise errors.RepoToolsDBStructError(
                "DB structure is not right: table _dbinfo missing"
            )
    ...
    def clean_db(self, sqlite_del=True):
        ...
        if self.db_type == "sqlite" and not self.in_ram and sqlite_del:
            ...
            self.connection = sqlite3.connect(
                self.db_path, timeout=self.timeout, detect_types=sqlite3.PARSE_DECLTYPES
            )
            self.cursor = self.connection.cursor()
        else:
            ...
            self.cursor.execute(
                "DROP TABLE IF EXISTS pullrequest_comment_reactions CASCADE;"
            )
            ...
            self.cursor.execute(
                "DROP TABLE IF EXISTS package_version_downloads CASCADE;"
            )
            ...
            self.cursor.execute(
                "DROP TABLE IF EXISTS filtered_deps_packageedges CASCADE;"
            )
            ...
            self.cursor.execute(
                "DROP TABLE IF EXISTS commit_comment_reactions CASCADE;"
            )
    ...
    def fill_db(self):
        ...
        for f in self.fillers:
            if not f.done:
                while (
                    not f.done
                ):  # condition on f.reexec not needed, present in error handling
                    if f.reexec > 0:
                        self.logger.info(
                            "Reexecution of filler {}: {}".format(f.name, f.reexec)
                        )
                    f.prepare()
                    ...
                    if not f.done:
                        try:
                            ...
                            self.logger.info(
                                "AdminShutdown detected for query, redoing query"
                            )
                            self.reconnect()
                            req = f.check_requirements()
                        if not req:
        ...
        if self.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    ...
                    if url_cleaned is not None
                ),
            )
            extras.execute_batch(
                self.cursor,
                """ UPDATE urls SET cleaned_url=id WHERE url=%s ;""",
                (
                    (url_cleaned,)
                    for url, url_cleaned, source_root_id in url_list
                    if url_cleaned is not None
            ...
            extras.execute_batch(
                self.cursor,
                """ UPDATE urls SET source=(SELECT id FROM sources WHERE name=%s),source_root=%s WHERE url=%s ;""",
                (
                    (
                        source,
                        source_root_id,
            ...
            self.cursor.executemany(
                """ UPDATE urls SET cleaned_url=id WHERE url=?;""",
                (
                    (url_cleaned,)
                    for url, url_cleaned, source_root_id in url_list
                    if url_cleaned is not None
            ...
            self.cursor.executemany(
                """ UPDATE urls SET source=(SELECT id FROM sources WHERE name=?),source_root=? WHERE url=?;""",
                (
                    (
                        source,
                        source_root_id,
    ...
    def register_url(
        self, repo_url, source="manual input", repo_id=None, clean_info=None
    ):  # DEPRECATED
        """
    ...
    def register_packages(
        self, source, package_list, autocommit=True, update_urls=False
    ):
        """
        ...
        if self.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    ...
                    for pid, name, c_at, url, a_at in package_list
                ),
            )
            if update_urls:
                extras.execute_batch(
    ...
    def get_repo_id(self, owner, name, source):
        ...
        if isinstance(source, str):
            ...
            if len(ans) == 0:
                ...
                raise ValueError(
                    "Project {}/{} could not be identified without specifying the source, {} projects found".format(
                        owner, name, len(ans)
                    )
                )
    ...
    def get_user_id(self, user_id=None, identity_id=None):
        ...
        if user_id is None and identity_id is None:
            ...
            if len(ans) == 1:
                ...
                raise ValueError(
                    "Several identities corresponding to several users ({}) fit this identity:{}".format(
                        len(ans), identity_id
                    )
                )
            ...
            raise ValueError(
                "User not found or not parsed, user_id: {},identity_id: {}".format(
                    user_id, identity_id
                )
            )
    ...
    def fill_authors(self, commit_info_list, autocommit=True):
        ...
        if not tracked_data["empty"]:
            ...
            if self.db_type == "postgres":
                self.cursor.execute(
                    """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(%s,'users',%s) ;""",
                    (repo_id, latest_commit_time),
                )
            else:
                self.cursor.execute(
                    """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(?,'users',?) ;""",
                    (repo_id, latest_commit_time),
                )
    ...
    def fill_commits(self, commit_info_list, autocommit=True):
        ...
        if not tracked_data["empty"]:
            ...
            if self.db_type == "postgres":
                self.cursor.execute(
                    """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(%s,'commits',%s) ;""",
                    (repo_id, latest_commit_time),
                )
            else:
                self.cursor.execute(
                    """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(?,'commits',?) ;""",
                    (repo_id, latest_commit_time),
                )
    ...
    def get_source_info(self, source=None, repo_id=None):
        ...
        if isinstance(source, str):
            if self.db_type == "postgres":
                self.cursor.execute(
                    "SELECT id,url_root FROM sources WHERE name=%s;", (source,)
                )
            else:
                self.cursor.execute(
                    "SELECT id,url_root FROM sources WHERE name=?;", (source,)
                )
            ans = self.cursor.fetchone()
        elif isinstance(source, int):
            if self.db_type == "postgres":
                self.cursor.execute(
                    "SELECT id,url_root FROM sources WHERE id=%s;", (source,)
                )
            else:
                self.cursor.execute(
                    "SELECT id,url_root FROM sources WHERE id=?;", (source,)
                )
            ans = self.cursor.fetchone()
        ...
        if ans is None:
            raise ValueError(
                "Unregistered or unparsed source {} or repo_id {}".format(
                    source, repo_id
                )
            )
        else:
    ...
    def get_last_star(self, source=None, repo=None, owner=None, repo_id=None):
        ...
        if repo_id is None:
            if owner is None or source is None or repo is None:
                raise SyntaxError(
                    "Invalid information for repo, source:{}, owner:{}, name:{}".format(
                        source, owner, repo
                    )
                )
            else:
    ...
    def count_stars(self, source=None, repo=None, owner=None, repo_id=None):
        ...
        if self.db_type == "postgres":
            self.cursor.execute(
                """SELECT COUNT(*) FROM stars WHERE repo_id=%s;""", (repo_id,)
            )
        else:
            self.cursor.execute(
                """SELECT COUNT(*) FROM stars WHERE repo_id=?;""", (repo_id,)
            )
    ...
    def count_forks(self, source=None, repo=None, owner=None, repo_id=None):
        ...
        if self.db_type == "postgres":
            self.cursor.execute(
                """SELECT COUNT(*) FROM forks WHERE forked_repo_id=%s;""", (repo_id,)
            )
        else:
            self.cursor.execute(
                """SELECT COUNT(*) FROM forks WHERE forked_repo_id=?;""", (repo_id,)
            )
        ans = self.cursor.fetchone()[0]  # When no count, result is (None,)
        if ans is None:
            return 0
    ...
    def clean_null_updates(
        self, table, repo_id=None, identity_id=None, autocommit=True
    ):
        """
    ...
    def set_cloned(self, repo_id, autocommit=True):
        ...
        if self.db_type == "postgres":
            self.cursor.execute(
                """UPDATE repositories SET cloned=true WHERE id=%s;""", (repo_id,)
            )
        else:
            self.cursor.execute(
                """UPDATE repositories SET cloned=1 WHERE id=?;""", (repo_id,)
            )
        if autocommit:
            self.connection.commit()
    ...
    def merge_identities(
        ...
        if user_id != old_user_id2:
            ...
            if self.db_type == "postgres":
                self.cursor.execute(
                    """UPDATE identities SET user_id=%s WHERE user_id=%s;""",
                    (user_id, old_user_id2),
                )
            else:
                self.cursor.execute(
                    """UPDATE identities SET user_id=? WHERE user_id=?;""",
                    (user_id, old_user_id2),
                )
            ...
            if self.db_type == "postgres":
                self.cursor.execute(
                    """ DELETE FROM users WHERE id=%s;""", (old_user_id2,)
                )
            else:
                self.cursor.execute(
                    """ DELETE FROM users WHERE id=?;""", (old_user_id2,)
                )
        else:
    ...
    def plan_repo_merge(
        ...
        # checks
        if (new_id is None and (new_owner is None or new_name is None)) or (
            obsolete_id is None and (obsolete_owner is None or obsolete_name is None)
        ):
            raise SyntaxError(
                "Insufficent info provided for merging repositories (id,source,owner,name): \n new ({},{},{},{}) \n obsolete ({},{},{},{})".format(
                    new_id,
                    new_source,
                    new_owner,
                    new_name,
    ...
    def batch_merge_repos(self):
        ...
        self.logger.info(
            "Batch merging {} repos ({} unique)".format(
                len(merge_list), len(set([tuple(d.items()) for d in merge_list]))
            )
        )
    ...
    def merge_repos(
        ...
        if obsolete_id is None:
            obsolete_id = self.get_repo_id(
                source=obsolete_source, owner=obsolete_owner, name=obsolete_name
            )
            if obsolete_id is None:
                if fail_on_no_repo_id:
                    raise ValueError(
                        "Repository to be merged {}/{}/{} not found in DB. Destination repo: {}/{}/{} ({})".format(
                            obsolete_source,
                            obsolete_owner,
                            obsolete_name,
                            new_source,
        ...
        if obsolete_owner is None:
            if self.db_type == "postgres":
                self.cursor.execute(
                    "SELECT owner,name FROM repositories WHERE id=%s;", (obsolete_id,)
                )
            else:
                self.cursor.execute(
                    "SELECT owner,name FROM repositories WHERE id=?;", (obsolete_id,)
                )
            ans = self.cursor.fetchone()
            if ans[0] is None:
        ...
        if new_id == obsolete_id:
            self.logger.info(
                "Repositories to be merged already match(id,source,owner,name): \n new ({},{},{},{}) \n obsolete ({},{},{},{})".format(
                    new_id,
                    new_source,
                    new_owner,
                    new_name,
            ...
            self.logger.info(
                "Repositories to be merged (id,source,owner,name): \n new ({},{},{},{}) \n obsolete ({},{},{},{})".format(
                    new_id,
                    new_source,
                    new_owner,
                    new_name,
            ...
            if (
                new_id is None
            ):  # in this case we know new_owner and new_name were provided
                self.register_source(source=merging_reason_source)
                ...
                obsolete_source_id, obsolete_url_root = self.get_source_info(
                ...
                if self.db_type == "postgres":
                    self.cursor.execute(
                        "SELECT name FROM sources WHERE id=%s;", (new_source_id,)
                    )
                else:
                    self.cursor.execute(
                        "SELECT name FROM sources WHERE id=?;", (new_source_id,)
                    )
                new_source_name = self.cursor.fetchone()[0]

                if self.db_type == "postgres":
                    self.cursor.execute(
                        "SELECT name FROM sources WHERE id=%s;", (obsolete_source_id,)
                    )
                else:
                    self.cursor.execute(
                        "SELECT name FROM sources WHERE id=?;", (obsolete_source_id,)
                    )
                obsolete_source_name = self.cursor.fetchone()[0]

                # add url
                self.register_urls(
                    url_list=[(url, url, new_source_id)], source=merging_reason_source
                )
                ...
                if self.db_type == "postgres":
                    ...
                    if ans is None:
                        old_url = "{},{},{}".format(
                            obsolete_source, obsolete_owner, obsolete_name
                        )
                    else:
                        old_url = ans[0]
                ...
                if os.path.exists(
                    ...
                    if not os.path.exists(
                        os.path.join(
                            self.clone_folder, new_source_name, new_owner, new_name
                        )
                    ):
                        if not os.path.exists(
                            os.path.join(self.clone_folder, new_source_name, new_owner)
                        ):
                            os.makedirs(
                                os.path.join(
                                    self.clone_folder, new_source_name, new_owner
                                )
                            )
                        shutil.move(
                            os.path.join(
                ...
                if self.db_type == "postgres":
                    ...
                    self.cursor.execute(
                        "DELETE FROM repositories WHERE id=%s;", (obsolete_id,)
                    )
                    ...
                    self.cursor.execute(
                        "DELETE FROM repositories WHERE id=?;", (obsolete_id,)
                    )
...
class ComputationDB(object):
    ...
    def get_measure_id(
        self, measure=None, params=None, check=False, create_if_absent=True
    ):
        if isinstance(measure, int):
            if check:
                self.cursor.execute(
                    "SELECT id FROM measures WHERE id=? AND db_id=?;",
                    (measure, self.db_id),
                )
                ans = self.cursor.fetchone()
                if ans is None:
                    raise ValueError(
                        "(db_id {}) No such measure id in computation_db: {}".format(
                            self.db_id, measure
                        )
                    )
            return measure
            ...
            self.cursor.execute(
                "SELECT id FROM measures WHERE name=? AND params=? AND db_id=?;",
                (measure, params, self.db_id),
            )
            ans = self.cursor.fetchone()
            if ans is None:
                if not create_if_absent:
                    raise ValueError(
                        "(db_id {}) No such measure in computation_db: {}, {}".format(
                            self.db_id, measure, params
                        )
                    )
                else:
                    self.cursor.execute(
                        "INSERT INTO measures(name,params,db_id) VALUES(?,?,?);",
                        (measure, params, self.db_id),
                    )
                    row_id = self.cursor.lastrowid
                    return row_id
    ...
    def read(
        ...
        start_time, end_time = self.format_times(
    ...
    def batch_write(self, table_name, measure, params=None, data=None, autocommit=True):
        ...
        if table_name not in ["repositories", "users"]:
            ...
            self.cursor.executemany(
                ...
                (
                    (measure_id, oid, measured_at, val)
                    for (oid, measured_at, val) in data
                ),
            )
    ...
    def is_completed(self, measure=None, params=None):
        if isinstance(measure, int):
            self.cursor.execute(
                "SELECT completed_at FROM measures WHERE id=? AND completed_at IS NOT NULL;",
                (measure,),
            )
        else:
            params = self.format_params(params)
            self.cursor.execute(
                "SELECT completed_at FROM measures WHERE name=? AND params=? AND db_id=? AND completed_at IS NOT NULL;",
                (measure, params, self.db_id),
            )
        ans = self.cursor.fetchone()
        if ans is None:
