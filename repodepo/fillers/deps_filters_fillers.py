    ...
    def prepare(self):
        ...
        if self.input_list is not None:
            ...
            if os.path.exists(filepath):
                ...
                self.logger.info(
                    f"""No such file: {filepath}, skipping {self.__class__}"""
                )
                self.done = True
                self.input_list = []
        else:
    ...
    def fill_filters(self):
        if self.db.db_type == "postgres":
            extras.execute_batch(
				...
				""",
                (
                    {"pname": pname, "reason": self.reason, "psource": s}
                    for s, pname in self.input_list
                ),
            )
        else:
        ...
        if self.db.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    ...
                    for s, rowner, rname in [
                        (s, i.split("/")[0], i.split("/")[1])
                        for s, i in self.input_list
                    ]
                ),
            )
        else:
...
class PackageEdgesDepsFilter(PackageDepsFilter):
        ...
        if self.db.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    ...
                    for psource1, pname1, psource2, pname2 in self.input_list
                ),
            )
        else:
            self.db.cursor.executemany(
    ...
    def parse_element(self, elt):
        ...
        if len(elt) == 2:
            return tuple(
                list(PackageDepsFilter.parse_element(self, elt[0]))
                + list(PackageDepsFilter.parse_element(self, elt[1]))
            )
        elif len(elt) == 4:
            return tuple(
                list(PackageDepsFilter.parse_element(self, elt[:2]))
                + list(PackageDepsFilter.parse_element(self, elt[2:]))
            )
        raise ValueError("{} not parsable".format(elt))
...
class RepoEdgesDepsFilter(PackageDepsFilter):
        ...
        if self.db.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    ...
                    for psource1, rowner1, rname1, psource2, rowner2, rname2 in [
                        (
                            s1,
                            r1.split("/")[0],
                            r1.split("/")[1],
                        ...
                        for s1, r1, s2, r2 in self.input_list
                    ]
                ),
            )
        else:
    ...
    def parse_element(self, elt):
        ...
        if len(elt) == 2:
            return tuple(
                list(RepoDepsFilter.parse_element(self, elt[0]))
                + list(RepoDepsFilter.parse_element(self, elt[1]))
            )
        elif len(elt) == 4:
            return tuple(
                list(RepoDepsFilter.parse_element(self, elt[:2]))
                + list(RepoDepsFilter.parse_element(self, elt[2:]))
            )
        elif len(elt) == 6:
            return tuple(
                list(RepoDepsFilter.parse_element(self, elt[:3]))
                + list(RepoDepsFilter.parse_element(self, elt[3:]))
            )
        raise ValueError("{} not parsable".format(elt))
...
class AutoPackageEdges2Cycles(PackageEdgesDepsFilter):
    ...
    def __init__(self, **kwargs):
        PackageEdgesDepsFilter.__init__(
            self,
            input_list=[],
            reason="autoremove of 2-cycles in package space base on earliest package creation date",
            **kwargs,
        )
...
class AutoRepoEdges2Cycles(RepoEdgesDepsFilter):
    ...
    def __init__(self, **kwargs):
        RepoEdgesDepsFilter.__init__(
            self,
            input_list=[],
            reason="autoremove of 2-cycles in repo space base on earliest repo or package creation date",
            **kwargs,
        )
    ...
    def get_input_list(self):
        ...
        self.input_list = [
            (s1, "/".join([r1o, r1n]), s2, "/".join([r2o, r2n]))
            for (s1, r1o, r1n, s2, r2o, r2n) in self.db.cursor.fetchall()
        ]
        self.input_list = [self.parse_element(e) for e in self.input_list]
...
class FiltersFolderFiller(fillers.Filler):
    ...
    def after_insert(self):
        self.db.add_filler(
            PackageDepsFilter(
                input_file=os.path.join(self.input_folder, self.packages_file),
                in_data_folder=self.in_data_folder,
            )
        )
        self.db.add_filler(
            RepoDepsFilter(
                input_file=os.path.join(self.input_folder, self.repos_file),
                in_data_folder=self.in_data_folder,
            )
        )
        self.db.add_filler(
            RepoEdgesDepsFilter(
                input_file=os.path.join(self.input_folder, self.repoedges_file),
                in_data_folder=self.in_data_folder,
            )
        )
        self.db.add_filler(
            PackageEdgesDepsFilter(
                input_file=os.path.join(self.input_folder, self.packageedges_file),
                in_data_folder=self.in_data_folder,
            )
        )
...
class FiltersLibFolderFiller(FiltersFolderFiller):
    def __init__(self, **kwargs):
        FiltersFolderFiller.__init__(
            self,
            input_folder=os.path.abspath(
                os.path.join(
                    os.path.dirname(os.path.dirname(fillers.__file__)),
                    "data",
                    "filters",
                )
            ),
...
class DepsManualChecksFiller(fillers.Filler):
    def __init__(
        self,
        filename="deps_manualchecks.yml",
        timestamp=None,
        cycle_limit=100,
        only_timestamp=True,
        **kwargs,
    ...
    def apply(self):
        ...
        for n in net_list:
            if s[n + "_filtered"]["cycles"]["nb_cycles"] > 0:
                ...
                raise ValueError(
                    f"""Pending manual checks for dependencies for network {s[n+'_filtered']}, see output file: {filepath}"""
                )
            else:
                self.logger.info(
                    f"""Passed dependency network checks for network {s[n+'_filtered']}, no cycles found"""
                )
