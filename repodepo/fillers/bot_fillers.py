    ...
    def prepare(self):
        ...
        for b in self.bot_list:
            if isinstance(b, str):
                ...
                raise SyntaxError(
                    "In provided botlist, an element has more than 2 items (should be (<identity_type>,<bot>) or <bot> :{}".format(
                        b
                    )
                )
...
class MGBotFiller(BotFileFiller):
    ...
    def __init__(
        self,
        botfile_url="https://zenodo.org/record/4000388/files/groundtruthbots.csv.gz?download=1",
        **kwargs,
    ):
        self.botfile_url = botfile_url
        BotFileFiller.__init__(
            self, bot_file="groundtruthbots_formatted.csv", header=True, **kwargs
        )
    ...
    def prepare(self):
        ...
        if not os.path.exists(
            os.path.join(self.data_folder, "groundtruthbots_formatted.csv")
        ):
            if not os.path.exists(
                os.path.join(self.data_folder, "groundtruthbots.csv")
            ):
                if not os.path.exists(
                    os.path.join(self.data_folder, "groundtruthbots.csv.gz")
                ):
                    self.download(
                        url=self.botfile_url,
                        destination=os.path.join(
                            self.data_folder, "groundtruthbots.csv.gz"
                        ),
                    )
                self.ungzip(
                    orig_file=os.path.join(self.data_folder, "groundtruthbots.csv.gz"),
                    destination=os.path.join(self.data_folder, "groundtruthbots.csv"),
                )
            self.parse(
                orig_file=os.path.join(self.data_folder, "groundtruthbots.csv"),
                destination=os.path.join(
                    self.data_folder, "groundtruthbots_formatted.csv"
                ),
            )
...
class BotsManualChecksFiller(fillers.Filler):
    ...
    def prepare(self):
        ...
        self.to_check = [
            {
                "user_id": row["user_id"],
                "identity_type": row["login"].split("/")[0],
                "identity": row["login"].split("/")[1],
            }
    ...
    def checks(self, silent=False):
        ...
        if self.blocking:
            self.db.cursor.execute(
                "SELECT COUNT(*) FROM _bots_manual_check WHERE is_bot IS NULL;"
            )
            ans = self.db.cursor.fetchone()
            if ans[0] != 0:
                if not self.auto_update:
                    raise ValueError(
                        "Pending manual checks for bots/invalid identities in _bots_manual_checks table"
                    )
                else:
                    self.cycle()
            elif not silent:
                self.logger.info(
                    "Passed bot checks, no further identities to be checked manually"
                )
                return
    ...
    def cycle(self):
        ...
        while bots_decision_remaining != 0:
            ...
            if bots_decision_remaining != before_val:
                self.logger.info(
                    f"Bots manual checks remaining: {bots_decision_remaining}"
                )
