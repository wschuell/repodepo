...
class GithubFiller(fillers.Filler):
    ...
    def __init__(
        self,
        querymin_threshold=50,
        per_page=100,
        env_apikey="GITHUB_API_KEY",
        workers=1,
        identity_type="github_login",
        no_unauth=False,
        api_keys_file="github_api_keys.txt",
        api_keys=None,
        fail_on_wait=False,
        start_offset=None,
        retry=False,
    ...
    def prepare(self):
        ...
        if self.db.db_type == "postgres":
            self.db.cursor.execute(
                """ INSERT INTO identity_types(name) SELECT %(id_type)s WHERE NOT EXISTS (SELECT id FROM identity_types WHERE name=%(id_type)s) ON CONFLICT DO NOTHING;""",
    ...
    def set_api_keys(self, reset=False):
        ...
        if os.path.exists(api_keys_file):
            ...
            os.path.join(os.environ[homepath()], ".repo_tools", self.api_keys_file)
        ):
            with open(
                os.path.join(os.environ[homepath()], ".repo_tools", self.api_keys_file),
                "r",
        ...
        try:
            ...
            pass
        self.api_keys = [
            ak for ak in self.api_keys if ak != ""
        ]  # removing empty api keys, can come from e.g. trailing newlines at end of parsed file
        api_keys = []
        ...
        if not len(self.api_keys):
            self.logger.info(
                "No API keys found for {}, using default anonymous (but limited and only REST, no GQL) authentication.\nLooking for api_keys_file in db.data_folder, then in current folder, then in ~/.repo_tools/.\nAdding content of environment variables {} and REPOTOOLS_{}.".format(
                    self.env_apikey.split("_")[0], self.env_apikey, self.env_apikey
                )
            )
        ...
        if random_pick:
            while True:
                active_requesters = [
                    ...
                    if self.get_rate_limit(rq) > self.querymin_threshold
                ]
                if len(active_requesters):
                    ...
                    if self.fail_on_wait:
                        raise IOError(
                            "All {} API keys are below the min remaining query threshold. Estimated time to earliest reset: {}s ({})".format(
                                len(requesters), time_to_reset, date_reset
                            )
                        )
                    else:
                        self.logger.info(
                            "Waiting for reset of at least one github requester, sleeping {} seconds".format(
                                time_to_reset + 1
                            )
                        )
            ...
            while True:
                for i, rq in enumerate(requesters):
                    self.logger.debug(
                        "Using github requester {}, {} queries remaining".format(
                            i, self.get_rate_limit(rq)
                        )
                    )
                    # time.sleep(0.5)
                ...
                if any(
                    (
                        (self.get_rate_limit(rq) > self.querymin_threshold)
                        for rq in requesters
                    )
                ):
                    continue
            ...
            for r in self.db.get_repo_list(option="starinfo"):
                ...
                if (
                    ...
                    or (
                        ...
                        and time.time() - created_at.timestamp() > force
                    )
                    or (retry and not success)
                ):
            ...
            try:
                ...
                while len(repo_list):
                    ...
                    if new_repo:
                        ...
                        new_repo = False
                        self.logger.info(
                            "Filling stars for repo {}/{} ({}/{})".format(
                                owner, repo_name, repo_nb, total_repos
                            )
                        )
                    requester = next(requester_gen)
                    try:
                        repo_apiobj = requester.get_repo(
                            "{}/{}".format(owner, repo_name)
                        )
                    except UnknownObjectException:
                        self.logger.info(
                            "No such repository: {}/{} ({}/{})".format(
                                owner, repo_name, repo_nb, total_repos
                            )
                        )
                        ...
                        if repo_apiobj.watchers_count == 0:
                            ...
                            db.insert_update(
                                repo_id=repo_id, table="stars", success=True
                            )
                            db.connection.commit()
                            repo_list.pop(0)
                            new_repo = True
                        ...
                        while self.get_rate_limit(requester) > self.querymin_threshold:
                            ...
                            sg_list = list(
                                repo_apiobj.get_stargazers_with_dates().get_page(
                                    int(nb_stars / self.per_page)
                                )
                            )

                            if nb_stars < self.per_page * (
                                int(nb_stars / self.per_page)
                            ) + len(sg_list):
                                # if in_thread:
                ...
                if in_thread:
                    self.logger.error(
                        "Exception in stars {}/{}/{}: \n {}: {}".format(
                            source, owner, repo_name, e.__class__.__name__, e
                        )
                    )
                db.log_error(
                    "Exception in stars {}/{}/{}: \n {}: {}".format(
                ...
                raise Exception(
                    "Exception in stars {}/{}/{}".format(source, owner, repo_name)
                ) from e
            finally:
...
class GHLoginsFiller(GithubFiller):
    ...
    def fill_gh_logins(
        self, info_list=None, workers=1, in_thread=False, user_nb=None, total_users=None
    ):
        """
        ...
        if workers == 1:
            ...
            try:
                ...
                for infos in info_list:
                    ...
                    self.logger.info(
                        "Filling gh login for user id {} ({}/{})".format(
                            identity_id, user_nb, total_users
                        )
                    )
                    requester = next(requester_gen)
                    try:
                        ...
                        self.logger.info(
                            "No such repository: {}/{}".format(repo_owner, repo_name)
                        )
                        ...
                        if commit_apiobj is None:
                            ...
                            if commit_apiobj.author is None:
                                login = None
                                self.logger.info(
                                    "No login available for user id {} ({}/{})".format(
                                        identity_id, user_nb, total_users
                                    )
                                )
                            else:
                                try:
                                    ...
                                    self.logger.info(
                                        "Found login {} for user id {} ({}/{})".format(
                                            login, identity_id, user_nb, total_users
                                        )
                                    )
                                except (UnknownObjectException, IncompletableObject):
                                    self.logger.info(
                                        "No login available for user id {} ({}/{}), uncompletable object error".format(
                                            identity_id, user_nb, total_users
                                        )
                                    )
                                    login = None
                            self.set_gh_login(
                                ...
                                login=login,
                                reason="Email/login match through github API for commit {}".format(
                                    commit_sha
                                ),
                            )
                    user_nb += 1
                ...
                if in_thread:
                    self.logger.error(
                        "Exception in getting login {}: \n {}: {}".format(
                            identity_id, e.__class__.__name__, e
                        )
                    )
                db.log_error(
                    "Exception in getting login {}: \n {}: {}".format(
                ...
                raise Exception(
                    "Exception in getting login {}".format(identity_id)
                ) from e
            finally:
                if in_thread and "db" in locals():
                    db.cursor.close()
...
class ForksFiller(GithubFiller):
    ...
    def fill_forks(
        ...
        if workers == 1:
            ...
            try:
                ...
                while len(repo_list):
                    ...
                    try:
                        ...
                        if repo_apiobj.forks_count == 0:
                            ...
                            db.insert_update(
                                repo_id=repo_id, table="forks", success=True
                            )
                            db.connection.commit()
                            repo_list.pop(0)
                            new_repo = True
                        ...
                        while self.get_rate_limit(requester) > self.querymin_threshold:
                            ...
                            # sg_list = list(repo_apiobj.get_stargazers_with_dates()[nb_stars:nb_stars+per_page])
                            sg_list = list(
                                repo_apiobj.get_forks().get_page(
                                    int(nb_forks / self.per_page)
                                )
                            )
                            forks_list = [
                                {
                                    ...
                                    "repo": repo_name,
                                    "owner": owner,
                                    "repo_fullname": sg.full_name,
                                    "created_at": sg.created_at,
                                }
                                for sg in sg_list
                            ]

                            if nb_forks < self.per_page * (
                                int(nb_forks / self.per_page)
                            ) + len(sg_list):
                                # if in_thread:
                                # if db.db_type == 'sqlite' and in_thread:
                ...
                raise Exception(
                    "Exception in forks {}/{}/{}".format(source, owner, repo_name)
                ) from e
            finally:
                if in_thread and "db" in locals():
                    db.cursor.close()
    ...
    def fill_fork_ranks(self, step=1):
        ...
        if rowcount > 0:
            self.logger.info(
                "Filled {} missing indirect fork relations".format(rowcount)
            )
...
class FollowersFiller(GithubFiller):
    ...
    def fill_followers(
        ...
        if workers == 1:
            login_id, login, identity_type_id = (
                None,
                None,
                None,
            ...
            try:
                ...
                while len(login_list):
                    ...
                    if new_login:
                        ...
                        new_login = False
                        self.logger.info(
                            "Filling followers for login {} ({}/{})".format(
                                login, user_nb, total_users
                            )
                        )
                    requester = next(requester_gen)
                    try:
                        ...
                        self.logger.info(
                            "No such login: {} ({}/{})".format(
                                login, user_nb, total_users
                            )
                        )
                        db.insert_update(
                            identity_id=login_id, table="followers", success=False
                        )
                        login_list.pop(0)
                        new_login = True
                        user_nb += 1
                        ...
                        if login_apiobj.followers == 0:
                            self.logger.info(
                                "No followers for login: {} ({}/{})".format(
                                    login, user_nb, total_users
                                )
                            )
                            db.insert_update(
                                identity_id=login_id, table="followers", success=True
                            )
                            db.connection.commit()
                            login_list.pop(0)
                            new_login = True
                        ...
                        while self.get_rate_limit(requester) > self.querymin_threshold:
                            ...
                            sg_list = list(
                                login_apiobj.get_followers().get_page(
                                    int(nb_followers / self.per_page)
                                )
                            )

                            if nb_followers < self.per_page * (
                                int(nb_followers / self.per_page)
                            ) + len(sg_list):
                                # if db.db_type == 'sqlite' and in_thread:
                                #   time.sleep(1+random.random()) # to avoid database locked issues, and smooth a bit concurrency
                                ...
                                self.logger.info(
                                    "Filled followers for login {} ({}/{}): {}".format(
                                        login, user_nb, total_users, nb_followers
                                    )
                                )
                                db.insert_update(
                                    identity_id=login_id,
                                    table="followers",
                                    success=True,
                                )
                                db.connection.commit()
                ...
                if in_thread:
                    self.logger.error(
                        "Exception in followers {}: \n {}: {}".format(
                            login, e.__class__.__name__, e
                        )
                    )
                db.log_error(
                    "Exception in followers {}: \n {}: {}".format(
