...
class Requester(object):
    ...
    def __init__(
        ...
        )  # Like on the API, reset time is last reset time, not future reset time
        ...
        self.transport = AIOHTTPTransport(
            ...
            headers={
                ...
                "Authorization": "{}{}".format(self.auth_header_prefix, self.api_key),
            },
        )
    ...
    def get_rate_limit(self, refresh=False):
        if (
            ...
            or self.refreshed_at + datetime.timedelta(seconds=self.refresh_time)
            <= datetime.datetime.now()
        ):
            query_str = """
                    query {
            ...
            try:
                ...
                if hasattr(e, "code") and e.code == 403:
                    self.logger.info(
                        "Error 403 detected when setting up key, sleeping for 60s and retrying"
                    )
                    time.sleep(60)
                    self.query(query_str)
                else:
    ...
    def format_query(self, gql_query, params):
        ...
        if "rateLimit" not in gql_query:
            ...
            gql_query = (
                "}".join(splitted_string[:-1]) + RL_query + "}" + splitted_string[-1]
            )
        return gql_query
        ...
        try:
            ...
            while not result_found:
                try:
                    ...
                    if (
                        e.__class__
                        in (asyncio.TimeoutError, concurrent.futures._base.TimeoutError)
                        or "TimeoutError" in str(e)
                        or "TimeoutError" in str(e.__class__)
                    ):
                        if retries_left > 0:
                            ...
                            raise e.__class__(
                                """TimeoutError happened more times than the set retries: {}. Rerun, maybe with higher value.
Original error message: {}""".format(
                    ...
                    ) and "You have exceeded a secondary rate limit. Please wait a few minutes before you try again." in str(
                        e
                    ):
                        if retries_left > 0:
                            self.logger.info(
                                f"Secondary rate limit exceeded, waiting {self.secondary_limit_wait}s"
                            )
                            ...
                            raise e.__class__(
                                """Secondary rate limit error happened more times than the set retries: {}. Rerun, maybe with higher value.
Original error message: {}""".format(
                                    retries, e
                                )
                            )
                        ...
                        hasattr(e, "errors")
                        and len(e.errors)
                        and "type" in e.errors[0].keys()
                        and e.errors[0]["type"] == "RATE_LIMITED"
                    ):
                        self.get_rate_limit()
                        ...
                        self.logger.info(
                            f"RATE_LIMITED error detected, sleeping until reset: {reset_time+1}s"
                        )
        ...
        self.reset_at = datetime.datetime.strptime(
            result["rateLimit"]["resetAt"], "%Y-%m-%dT%H:%M:%SZ"
        )
        self.reset_at = time.mktime(
            self.reset_at.timetuple()
        )  # converting to seconds to epoch; to have same format as REST API
    ...
    def paginated_query(self, gql_query, params=None, pageinfo_path=[], retries=None):
        ...
        while has_next_page:
            result = self.query(
                gql_query=gql_query, params=params, retries=retries, cp_params=False
            )
            ...
            yield copy.deepcopy(result), copy.deepcopy(
                page_info
            )  # copying so that any usage of results fields in the generator cannot be corrupted between 2 yields
...
class GHGQLFiller(github_rest.GithubFiller):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        github_rest.GithubFiller.__init__(
            self, identity_type=target_identity_type, max_reexec=max_reexec, **kwargs
        )
    ...
    def set_requesters(self, fetch_schema=False):
        ...
        for ak in self.api_keys:
            ...
            try:
                ...
                self.logger.info(
                    'API key starting with "{}" and of length {} not valid: {}:{}'.format(
                        ak[:6], len(ak), e.__class__, e
                    )
                )
            else:
                missing_scopes = []
                ...
                if len(missing_scopes) > 0:
                    self.logger.info(
                        'API key starting with "{}" missing scopes: {}'.format(
                            ak[:6], missing_scopes
                        )
                    )
                else:
    ...
    def get_requester(self, random_pick=True, in_thread=False, requesters=None):
        ...
        return github_rest.GithubFiller.get_requester(
            self, random_pick=random_pick, requesters=requesters
        )
    ...
    def fill_items(
        ...
        if workers == 1:
            elt_name, owner, repo_name, end_cursor, login = (
                None,
                None,
                None,
                None,
                None,
            )  # init values for the exception
            ...
            try:
                ...
                while len(elt_list):
                    ...
                    if self.queried_obj == "repo":
                        if self.sub_queried_obj != "repo":
                            ...
                            local_additional_query_attributes[
                                self.sub_queried_obj + "_id"
                            ] = sq_id
                            local_additional_query_attributes[
                                self.sub_queried_obj + "_gql_id"
                            ] = sq_gql_id
                        ...
                        (
                            ...
                            identity_id,
                            end_cursor_orig,
                        ) = current_elt  # source is here identity_type_id
                        repo_id = None
                        email = None
                        commit_sha = None
                        elt_name = login
                    ...
                    paginated_query = requester.paginated_query(
                        gql_query=self.query_string(),
                        params=params,
                        pageinfo_path=self.pageinfo_path,
                    )
                    try:
                        result, pageinfo = next(paginated_query)
                    except asyncio.TimeoutError as e:
                        if self.retry_fails_permanent:
                            err_text = "Timeout threshold reached {}, marking query as to be discarded {}: {}".format(
                                requester.retries, e.__class__, e
                            )
                    ...
                    if self.queried_obj == "repo":
                        checked_repo_owner, checked_repo_name = result["repository"][
                            "nameWithOwner"
                        ].split("/")
                        if (checked_repo_owner, checked_repo_name) != (
                            owner,
                            repo_name,
                        ):
                            to_be_merged = True
                            db.plan_repo_merge(
                                ...
                                merging_reason_source="Repo redirect detected on github GraphQL API when processing {}".format(
                                    self.items_name
                                ),
                            )
                            elt_name = "{}/{} ({}/{})".format(
                                checked_repo_owner, checked_repo_name, owner, repo_name
                            )
                        else:
                            to_be_merged = False

                    # detect 0 elts
                    parsed_result = self.parse_query_result(
                        result,
                        repo_id=repo_id,
                        identity_id=identity_id,
                    ...
                    if (
                        ...
                        and parsed_result[0]["repo_owner"] is None
                    ):
                        self.logger.info(
                            "No such repo: {}/{} for email {} ({}/{})".format(
                                owner, repo_name, elt_name, elt_nb, total_elt
                            )
                        )
                        self.insert_update(
                            db=db,
                        ...
                        self.queried_obj == "email"
                        and parsed_result[0]["commit_sha"] is None
                    ):
                        self.logger.info(
                            "No such commit: {} for email {} ({}/{})".format(
                                commit_sha, elt_name, elt_nb, total_elt
                            )
                        )
                        self.insert_update(
                            db=db,
                ...
                err_text = "Exception in {} {}: \n {}: {}".format(
                    self.items_name, elt_name, e.__class__.__name__, e
                )
                if (
                    ...
                    self.logger.error(
                        err_text + " retry_fails_permanent is set to True"
                    )
                if in_thread:
                    self.logger.error(err_text)
                db.log_error(err_text)
    ...
    def set_element_list(self, sub_queried_obj=None, queried_obj=None, items_name=None):
                    ...
                    ;""",
                        {
                            ...
                            "sub_queried_obj": sub_queried_obj,
                        },
                    )
                else:
                    self.db.cursor.execute(
        ...
        ################################################################
            ...
            raise NotImplementedError(
                f"set_element_list not implemented for queried_obj {queried_obj}"
            )
        ...
        **kwargs,
        ...
        for e in query_result["repository"]["stargazers"]["edges"]:
            ...
            try:
                ...
                self.logger.info(
                    "Result triggering error: {} \nError when parsing stars for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
            else:
        ...
        **kwargs,
        ...
        for e in query_result["repository"]["watchers"]["edges"]:
            d = {
                ...
                "repo_name": repo_name,
                "identity_type": self.target_identity_type,
            }
            try:
                d["watcher_login"] = e["node"]["login"]
            except (KeyError, TypeError) as err:
                self.logger.info(
                    "Result triggering error: {} \nError when parsing watchers for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
        ...
        **kwargs,
        ...
        for e in query_result["repository"]["forks"]["nodes"]:
            ...
            try:
                ...
                self.logger.info(
                    "Result triggering error: {} \nError when parsing forks for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
            ...
            if query_result["user"]["sponsorsListing"] is None:
                ...
                for e in query_result["user"]["sponsorshipsAsMaintainer"]["nodes"]:
                    ...
                    try:
                        ...
                        self.logger.info(
                            "KeyError when parsing sponsors_user for {}: {}".format(
                                user_login, err
                            )
                        )
                        continue
    ...
    def parse_query_result(self, query_result, identity_id, identity_type_id, **kwargs):
        ...
        for e in query_result["user"]["followers"]["edges"]:
            ...
            try:
                ...
                self.logger.info(
                    "KeyError when parsing followers for {}: {}".format(user_login, err)
                )
                continue
            else:
                ans.append(d)
    ...
    def parse_query_result(self, query_result, identity_id, identity_type_id, **kwargs):
        ...
        if query_result["user"] is not None:
            ...
            for e in query_result["user"]["sponsorshipsAsSponsor"]["nodes"]:
                d = {
                    "sponsor_id": identity_id,
                    "sponsor_login": user_login,
                    "identity_type_id": identity_type_id,
                }
                try:
        ...
        **kwargs,
        ...
        for e in query_result["repository"]["releases"]["nodes"]:
            ...
            try:
                ...
                self.logger.info(
                    "Result triggering error: {} \nError when parsing releases for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
...
class LoginsGQLFiller(GHGQLFiller):
    ...
    def parse_query_result(self, query_result, identity_id, **kwargs):
        ...
        if query_result["repository"] is None:
            ...
            ans["login"] = None
            ans["repo_owner"] = query_result["repository"]["nameWithOwner"].split("/")[
                1
            ]
            ...
            if query_result["repository"]["object"]["actor"] is None:
                ...
                ans["login"] = query_result["repository"]["object"]["actor"]["user"][
                    "login"
                ]
                ans["created_at"] = query_result["repository"]["object"]["actor"][
                    "user"
                ]["createdAt"]
    ...
    def insert_items(self, items_list, commit=True, db=None):
        ...
        for item in items_list:
            login = item["login"]
            reason = "Email/login match through {} for commit {}".format(
                self.__class__.__name__, item["commit_sha"]
            )
            ...
            if db.db_type == "postgres":
                if login is not None:
                    ...
                    db.merge_identities(
                        identity1=identity2,
                        identity2=identity_id,
                        autocommit=False,
                        reason=reason,
                    )
                db.cursor.execute(
                    """INSERT INTO table_updates(identity_id,table_name,success) VALUES(%s,'login',%s);""",
                    (identity_id, (login is not None)),
                )
            else:
                if login is not None:
                    db.cursor.execute(
                ...
                db.cursor.execute(
                    """INSERT INTO table_updates(identity_id,table_name,success) VALUES(?,'login',?);""",
                    (identity_id, (login is not None)),
                )
        if commit:
            db.connection.commit()
...
class CommitCommentReactionsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing commit_comments filler, necessary prior to commit_comment_reactions"
            )
            return False
        else:
            return True
        ...
        **kwargs,
        ...
        for e in [query_result["node"]]:
            ...
            try:
                ...
                self.logger.info(
                    "Result triggering error: {} \nError when parsing commit_comments for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
                ...
                if e["reactions"]["totalCount"] > 0:
                    for ee in e["reactions"]["nodes"]:
                        r = {
                            ...
                            "repo_name": repo_name,
                            "target_identity_type": self.target_identity_type,
                            "element_type": "commit_comment_reaction",
                        }
                        ...
                        try:
                            ...
                            self.logger.info(
                                "Result triggering error: {} \nError when parsing commit_comment_reactions for {}/{}: {}".format(
                                    e, repo_owner, repo_name, err
                                )
                            )
                            continue
...
class CommitCommentsGQLFiller(GHGQLFiller):
    ...
    def after_insert(self):
        if self.complete_reactions:
            self.db.add_filler(
                CommitCommentReactionsGQLFiller(**self.get_generic_kwargs())
            )
    ...
    def insert_items(self, items_list, commit=True, db=None):
        ...
        if len(items_list):
            db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT 'commit_comments' WHERE NOT EXISTS (SELECT 1 FROM full_updates fu WHERE fu.update_type='commit_comments' );"""
            )
        ...
        if db.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    i
                    for i in items_list
                    if i["element_type"] == "commit_comment_reaction"
                ),
            )
            # ;''',((s['created_at'],s['closed_at'],s['repo_id'],s['issue_number'],s['issue_title'],s['issue_text'],s['author_login'],s['author_login'],self.target_identity_type,self.target_identity_type) for s in items_list))
        else:
    ...
    def insert_reaction_updates(self, items_list, commit=True, db=None):
        ...
        for i in items_list:
            if i["element_type"] == "commit_comment":
                update_info = {
                    "commit_comment_id": i["commit_comment_id"],
        ...
        **kwargs,
        ...
        for e in query_result["repository"]["issues"]["nodes"]:
            ...
            try:
                ...
                self.logger.info(
                    "Result triggering error: {} \nError when parsing issues for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
        ...
        **kwargs,
        ...
        for e in query_result["repository"]["pullRequests"]["nodes"]:
            ...
            try:
                ...
                self.logger.info(
                    "Result triggering error: {} \nError when parsing pullrequests for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
...
class CompleteIssuesGQLFiller(IssuesGQLFiller):
    ...
    def __init__(
        self, init_page_size=6, secondary_page_size=6, complete_info=True, **kwargs
    ):
        IssuesGQLFiller.__init__(
            self,
            init_page_size=init_page_size,
            secondary_page_size=secondary_page_size,
            other_update_names=["issues"],
            **kwargs,
        )
    ...
    def after_insert(self):
        if self.complete_info:
            ...
            self.db.add_filler(
                IssueCommentReactionsGQLFiller(**self.get_generic_kwargs())
            )
                ...
                if e["reactions"]["totalCount"] > 0:
                    for ee in e["reactions"]["nodes"]:
                        ...
                        try:
                            ...
                            self.logger.info(
                                "Result triggering error: {} \nError when parsing issue_reactions for {}/{}: {}".format(
                                    e, repo_owner, repo_name, err
                                )
                            )
                            continue
                ...
                if e["comments"]["totalCount"] > 0:
                    for ee in e["comments"]["nodes"]:
                        ...
                        try:
                            ...
                            self.logger.info(
                                "Result triggering error: {} \nError when parsing issue_comments for {}/{}: {}".format(
                                    e, repo_owner, repo_name, err
                                )
                            )
                            continue
                        ...
                        if ee["reactions"]["totalCount"] > 0:
                            for eee in ee["reactions"]["nodes"]:
                                r = {
                                    ...
                                    "repo_name": repo_name,
                                    "target_identity_type": self.target_identity_type,
                                    "element_type": "issue_comment_reaction",
                                }
                                r["issue_number"] = d["issue_number"]
                                r["issue_gql_id"] = d["issue_gql_id"]
                                r["issue_comment_id"] = ee["databaseId"]
                                ...
                                try:
                                    ...
                                    self.logger.info(
                                        "Result triggering error: {} \nError when parsing issue_comment_reactions for {}/{}: {}".format(
                                            e, repo_owner, repo_name, err
                                        )
                                    )
                                    continue
    ...
    def insert_items(self, items_list, commit=True, db=None):
        ...
        self.insert_comment_reaction_updates(
            items_list=items_list, commit=commit, db=db
        )

        if len(items_list):
            db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT 'complete_issues'  WHERE NOT EXISTS (SELECT 1 FROM full_updates fu WHERE fu.update_type='complete_issues' );"""
            )
        ...
        if db.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    i
                    for i in items_list
                    if i["element_type"] == "issue_comment_reaction"
                ),
            )
    ...
    def insert_comment_reaction_updates(self, items_list, commit=True, db=None):
        ...
        for i in items_list:
            if i["element_type"] == "issue_comment":
                update_info = {
                    "issue_comment_id": i["issue_comment_id"],
                    "issue_comment_gql_id": i["issue_comment_gql_id"],
                }
                if i["comment_reactions_pageinfo"]["hasNextPage"]:
                    success = None
                    update_info["end_cursor"] = i["comment_reactions_pageinfo"][
                        "endCursor"
                    ]
                    db.insert_update(
...
class IssueReactionsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_issues filler, necessary prior to issue_reactions"
            )
            return False
        else:
            return True
...
class IssueCommentsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_issues filler, necessary prior to issue_comments"
            )
            return False
        else:
            return True
    ...
    def insert_items(self, db=None, **kwargs):
        ...
        if len(kwargs["items_list"]):
            db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT 'issue_comments'  WHERE NOT EXISTS (SELECT 1 FROM full_updates fu WHERE fu.update_type='issue_comments' );"""
            )
...
class IssueCommentReactionsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_issues filler, necessary prior to issue_comment_reactions"
            )
            return False
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing issue_comments filler, necessary prior to issue_comment_reactions"
            )
            return False
        else:
            return True
...
class IssueLabelsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_issues filler, necessary prior to issue_labels"
            )
            return False
        else:
            return True
...
class CompletePullRequestsGQLFiller(PullRequestsGQLFiller):
    ...
    def __init__(
        ...
        PullRequestsGQLFiller.__init__(
            self,
            init_page_size=init_page_size,
            secondary_page_size=secondary_page_size,
            other_update_names=["pullrequests"],
            **kwargs,
        )
                ...
                if e["labels"]["totalCount"] > 0:
                    for ee in e["labels"]["nodes"]:
                        r = {
                            ...
                            "repo_name": repo_name,
                            "target_identity_type": self.target_identity_type,
                            "element_type": "pullrequest_label",
                        }
                ...
                if e["reactions"]["totalCount"] > 0:
                    for ee in e["reactions"]["nodes"]:
                        r = {
                            ...
                            "repo_name": repo_name,
                            "target_identity_type": self.target_identity_type,
                            "element_type": "pullrequest_reaction",
                        }
                        r["pullrequest_number"] = d["pullrequest_number"]
                        r["pullrequest_gql_id"] = d["pullrequest_gql_id"]
                        try:
                            ...
                            self.logger.info(
                                "Result triggering error: {} \nError when parsing pullrequest_reactions for {}/{}: {}".format(
                                    e, repo_owner, repo_name, err
                                )
                            )
                            continue
                ...
                if e["comments"]["totalCount"] > 0:
                    for ee in e["comments"]["nodes"]:
                        r = {
                            ...
                            "repo_name": repo_name,
                            "target_identity_type": self.target_identity_type,
                            "element_type": "pullrequest_comment",
                        }
                        r["pullrequest_number"] = d["pullrequest_number"]
                        r["pullrequest_gql_id"] = d["pullrequest_gql_id"]
                        try:
                            ...
                            self.logger.info(
                                "Result triggering error: {} \nError when parsing pullrequest_comments for {}/{}: {}".format(
                                    e, repo_owner, repo_name, err
                                )
                            )
                            continue
                        ...
                        if ee["reactions"]["totalCount"] > 0:
                            for eee in ee["reactions"]["nodes"]:
                                r = {
                                    ...
                                    "repo_name": repo_name,
                                    "target_identity_type": self.target_identity_type,
                                    "element_type": "pullrequest_comment_reaction",
                                }
                                r["pullrequest_number"] = d["pullrequest_number"]
                                r["pullrequest_comment_id"] = ee["databaseId"]
                                r["pullrequest_comment_gql_id"] = ee["id"]
                                try:
                                    ...
                                    self.logger.info(
                                        "Result triggering error: {} \nError when parsing pullrequest_comment_reactions for {}/{}: {}".format(
                                            e, repo_owner, repo_name, err
                                        )
                                    )
                                    continue
    ...
    def insert_items(self, items_list, commit=True, db=None):
        ...
        if len(items_list):
            db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT 'complete_pullrequests' WHERE NOT EXISTS (SELECT 1 FROM full_updates fu WHERE fu.update_type='complete_pullrequests' );"""
            )
        ...
        if db.db_type == "postgres":
            extras.execute_batch(
                ...
                (
                    i
                    for i in items_list
                    if i["element_type"] == "pullrequest_comment_reaction"
                ),
            )
    ...
    def insert_reaction_updates(self, items_list, commit=True, db=None):
        ...
        for i in items_list:
            if i["element_type"] == "pullrequest":
                update_info = {
                    "pullrequest_id": i["pullrequest_number"],
    ...
    def insert_comment_reaction_updates(self, items_list, commit=True, db=None):
        ...
        for i in items_list:
            if i["element_type"] == "pullrequest_comment":
                update_info = {
                    "pullrequest_comment_id": i["pullrequest_comment_id"],
                ...
                if i["comment_reactions_pageinfo"]["hasNextPage"]:
                    ...
                    db.insert_update(
                        table="pullrequest_comment_reactions",
                        repo_id=i["repo_id"],
                        success=success,
                        info=update_info,
                    )
...
class PRReactionsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_pullrequests filler, necessary prior to pullrequest_reactions"
            )
            return False
        else:
            return True
...
class PRCommentsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_pullrequests filler, necessary prior to pullrequest_comments"
            )
            return False
        else:
            return True
    ...
    def insert_items(self, db=None, **kwargs):
        ...
        CompletePullRequestsGQLFiller.insert_comment_reaction_updates(
            self, db=db, **kwargs
        )

        if len(kwargs["items_list"]):
            db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT 'pullrequest_comments'  WHERE NOT EXISTS (SELECT 1 FROM full_updates fu WHERE fu.update_type='pullrequest_comments' );"""
            )
...
class PRCommentReactionsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_pullrequests filler, necessary prior to pullrequest_comment_reactions"
            )
            return False
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing pullrequest_comments filler, necessary prior to pullrequest_comment_reactions"
            )
            return False
        else:
            return True
...
class PRLabelsGQLFiller(GHGQLFiller):
    ...
    def check_requirements(self):
        ...
        if len(ans) == 0:
            self.logger.info(
                "missing complete_pullrequests filler, necessary prior to pullrequest_labels"
            )
            return False
        else:
            return True
            ...
            if qr["__typename"] == "User":
                try:
                    elt = {
                        ...
                        "login": qr["login"],
                        "sponsorsListing_createdat": qr["sponsorsListing"]["createdAt"],
                    }
                except KeyError:
                    elt = {
                        "identity_type": "github_login",
                        "login": qr["login"],
                        "sponsorsListing_createdat": None,
                    }
                ans.append(elt)
        return ans
            ...
            try:
                ...
                if len(parsed_result) == 0:
                    ...
                    while pageinfo["hasNextPage"]:
                        # raise ValueError
                        while (
                            requester.get_rate_limit() > self.querymin_threshold
                            and pageinfo["hasNextPage"]
                        ):
                            # insert results
                            self.insert_items(
                                items_list=parsed_result, commit=True, db=db
                            )
                            end_cursor = pageinfo["endCursor"]
                            if end_cursor is None:
                                end_cursor_json = None
                            ...
                            if not pageinfo["hasNextPage"]:
                                ...
                                # insert partial update with endcursor value and success NULL (?)
                                self.insert_update(
                                    db=db, success=None, info=end_cursor_json
                                )
                                db.connection.commit()
                                # continue query
                                try:
                                    ...
                                    if self.retry_fails_permanent:
                                        ...
                                        db.log_error(err_text)
                                        self.insert_update(
                                            db=db, success=True, info=end_cursor_json
                                        )
                                        elt_list.pop(0)
                                        break
                                    else:
                ...
                db.log_error(
                    "Exception in {}: \n {}: {}".format(
                        self.items_name, e.__class__.__name__, e
                    )
                )
        ...
        **kwargs,
        ...
        for e in query_result["repository"]["languages"]["edges"]:
            ...
            try:
                ...
                self.logger.info(
                    "Result triggering error: {} \nError when parsing languages for {}/{}: {}".format(
                        e, repo_owner, repo_name, err
                    )
                )
                continue
    ...
    def parse_query_result(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.info(
                "Error when parsing creation date for {}/{}: {}".format(
                    repo_owner, repo_name, err
                )
            )
        else:
            ans.append(d)
...
class UserCreatedAtGQLFiller(GHGQLFiller):
    ...
    def parse_query_result(self, query_result, identity_id, identity_type_id, **kwargs):
        ...
        try:
            ...
            self.logger.info(
                "KeyError when parsing creation date for {}: {}".format(user_login, err)
            )
        else:
            ans.append(d)
        return ans
...
class UserInfoGQLFiller(GHGQLFiller):
    ...
    def parse_query_result(self, query_result, identity_id, identity_type_id, **kwargs):
        ...
        try:
            ...
            if (
                query_result["user"]["location"] is not None
                and query_result["user"]["location"] != ""
            ):
                d["user_info"]["location"] = query_result["user"]["location"]
            for ad_name, ad_path in self.additional_data.items():
                if (
                    query_result["user"][ad_path] is not None
                    and query_result["user"][ad_path] != ""
                ):
            ...
            self.logger.info(
                "KeyError when parsing user info for {}: {}".format(user_login, err)
            )
        else:
            ans.append(d)
        return ans
        ...
        for s in items_list:
            if s["twitter"] is not None:
                ...
                db.merge_identities(
                    ...
                    autocommit=False,
                    record=True,
                    reason=f"""Twitter login {s['twitter']} corresponding to {s['identity_type']} {s['user_login']}""",
                )
    ...
    def parse_query_result(self, query_result, identity_id, identity_type_id, **kwargs):
        ...
        for e in query_result["user"]["organizations"]["edges"]:
            try:
                ...
                self.logger.info(
                    "KeyError when parsing organizations for {}: {}".format(
                        user_login, err
                    )
                )
            else:
...
class SingleQueryUserLanguagesGQLFiller(GHGQLFiller):
    ...
    def parse_query_result(self, query_result, identity_id, identity_type_id, **kwargs):
        ...
        ans_dict = dict()
        user_login = query_result["user"]["login"]
        for r in query_result["user"]["contributionsCollection"][
            "commitContributionsByRepository"
        ]:
            try:
                nb_commits_repo = r["contributions"]["totalCount"]
                total_size = sum(
                    [e["size"] for e in r["repository"]["languages"]["edges"]]
                )
                ...
                for e in r["repository"]["languages"]["edges"]:
                    ...
                    size = e["size"]
                    if total_size != 0:
                        factor = size * 1.0 / total_size
                    else:
                        factor = 1.0 / nb_lang_repo
                ...
                self.logger.info(
                    "KeyError when parsing languages for {}: {}".format(user_login, err)
                )
        return [
            {
            ...
            for lang, size in ans_dict.items()
        ]
    ...
    def parse_query_result(self, query_result, identity_id, identity_type_id, **kwargs):
        ...
        for qname, q in query_result.items():
            if qname == "rateLimit" or q is None:
                continue
            for elt in SingleQueryUserLanguagesGQLFiller.parse_query_result(
                self,
                query_result={"user": q},
                identity_id=identity_id,
                identity_type_id=identity_type_id,
