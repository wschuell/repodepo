...
class CratesFiller(generic.PackageFiller):
    ...
    def regenerate_crates_db(self, min_regen_date=None, force_pastdl=None):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Could not connect to fallback database: {self.fallback_db}"
                )
                raise
            ...
            try:
                ...
                self.logger.info(
                    f"Could not create database from fallback database: {self.fallback_db}"
                )
                raise
        ...
        if max_date is not None and (
            min_regen_date is None or max_date >= min_regen_date
        ):
            if not force_pastdl:
                crates_conn.close()
                return
        ...
        if not only_pastdl:
            self.download(
                ...
                destination=os.path.join(self.crates_folder, f"dump_{t_str}.tar.gz"),
            )
            self.untar(
                orig_file=os.path.join(
                    self.crates_folder,
                    datetime.datetime.now().strftime(f"dump_{t_str}.tar.gz"),
                ),
                destination=os.path.join(self.crates_folder, f"dump_{t_str}"),
            )

            subfolder = os.path.basename(
                glob.glob(os.path.join(self.crates_folder, f"dump_{t_str}", "*"))[0]
            )
            ...
            with open(
                os.path.join(
                    self.crates_folder, f"dump_{t_str}", subfolder, "schema.sql"
                ),
                "r",
            ) as f:
                ...
                for create_str in (
                    ...
                    schema_script = create_str.join(
                        [
                            (c if c.startswith(ifnot_str) or i == 0 else ifnot_str + c)
                    ...
                    for i, c in enumerate(schema_script.split(create_str)):
                        if i == 0:
                            ...
                            if obj_str == "TRIGGER":
                                ...
                                new_schema_script += (
                                    f"DROP {obj_str} IF EXISTS {typename} CASCADE;\n"
                                )
            ...
            with open(
                os.path.join(
                    self.crates_folder, f"dump_{t_str}", subfolder, "import.sql"
                ),
                "r",
            ) as f:
            ...
            extensions = [
                l.replace(start, "").replace(end, "")
                for l in schema_script.split("\n")
            ...
            self.exec_script_copyfrom(
                cur=crates_cur,
                script=import_script,
                folder=os.path.join(self.crates_folder, f"dump_{t_str}", subfolder),
                oneline_cmd=True,
            )
        ...
        for filepath in sorted(pastdl_files):
            ...
            self.exec_script_copyfrom(
                cur=crates_cur,
                script=pastdl_script,
                folder=pastdl_folder,
                oneline_cmd=False,
            )
        ...
        for cmd in cmd_list:
            if (
                ...
                if " from " in cmd:
                    ...
                    from_string = "FROM"
                table = [s for s in cmd.split(copy_string)[1].split(" ") if s != ""][
                    0
                ].replace('"', "")
                ...
                columns = tuple(
                    [
                        cl.replace('"', "").replace(" ", "")
                        for cl in cmd.split("(")[1].split(")")[0].split(",")
                    ]
                )
                for cl in columns:
                    check_sqlname_safe(cl)
                col_string = f"""({','.join(columns)})"""
                filepath = [s for s in cmd.split(from_string)[1].split(" ") if s != ""][
                    0
                ].replace("'", "")
                ...
                with open(os.path.join(folder, filepath), "r") as f:
                    ...
                    copy_cmd = cmd.replace(copy_string, "COPY").replace(
                        f"'{filepath}'", "STDIN"
                    )
    ...
    def apply(self):
        ...
        self.db.cursor.execute(
            """INSERT INTO full_updates(update_type) SELECT 'crates';"""
        )
    ...
    def prepare(self):
        ...
        self.db.cursor.execute(
            """SELECT COUNT(*) FROM full_updates WHERE update_type='crates';"""
        )
        ...
        if crates_fullupdates > 0 and not self.force:
            if self.min_regen_date is not None:
                ...
                date_check = (max_package is not None) and (
                    self.min_regen_date <= max_package
                )
            else:
                date_check = True
            if date_check:
        ...
        try:
            ...
            if self.packages_done:
                ...
                self.package_list = list(
                    self.get_packages_from_crates(conn=crates_conn)
                )
            self.db.connection.commit()
            if not self.force:
                if self.db.db_type == "postgres":
                    self.db.cursor.execute(
                        """SELECT MAX(p.created_at) FROM packages p
                ...
                if ans is not None and ans[0] is not None:
                    ...
                    if isinstance(last_created_at, str):
                        last_created_at = datetime.datetime.strptime(
                            last_created_at, "%Y-%m-%d %H:%M:%S"
                        )
                    self.package_list = [
                        (i, n, c_at, r)
                        for (i, n, c_at, r) in self.package_list
                        if c_at > last_created_at
                    ]
            ...
            if not self.only_packages:  # and (self.force or len(self.package_list)>0):
                if not self.versions_done:
                    self.package_version_list = list(
                        self.get_package_versions_from_crates(conn=crates_conn)
                    )
                    self.db.connection.commit()
                if not self.downloads_done:
                    self.package_version_download_list = (
                        self.get_package_version_downloads_from_crates(conn=crates_conn)
                    )
                ...
                if not self.deps_done:
                    self.package_deps_list = list(
                        self.get_package_deps_from_crates(conn=crates_conn)
                    )
                    self.db.connection.commit()
    ...
    def get_package_versions_from_crates(self, conn, limit=None):
        ...
        if limit is not None:
            ...
            if not isinstance(self.package_limit, int):
                raise ValueError(
                    "limit should be an integer, given {}".format(self.package_limit)
                )
            else:
                limit_str = "INNER JOIN (SELECT id FROM crates ORDER BY id LIMIT {}) c ON c.id=v.crate_id".format(
                    self.package_limit
                )
        else:
            limit_str = ""
        ...
        if limit is not None:
            ...
            if not isinstance(self.package_limit, int):
                ...
                limit_str = """INNER JOIN (SELECT id FROM crates ORDER BY id LIMIT {lim}) c
						...
						ON c2.id=d.crate_id""".format(
                    lim=self.package_limit
                )
        else:
            limit_str = ""
