...
class UserSnowballFiller(fillers.Filler):
    ...
    def __init__(self, table_list=None, **kwargs):
        ...
        for t in self.table_list:
            if t not in available_tables:
                raise ValueError(
                    "{} is not in available_tables for Filler UserSnowballFiller: {}".format(
                        t, available_tables
                    )
                )
    ...
    def fill_snowballsponsors(self):
        ...
        self.logger.info(
            "Filling {} new identities from sponsors".format(len(missing_logins))
        )
    ...
    def fill_newidentities(self, new_identities, table_name):
        ...
        self.db.connection.commit()
        self.logger.info(
            "Filled {} new users and {} new identity associations from table {}".format(
                nb_users, nb_identities, table_name
            )
        )
