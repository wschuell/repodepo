...
class MetaCRANFiller(generic.PackageFiller):
    ...
    def parse_metacran_info(self):
        ...
        if self.package_limit is not None:
            self.package_names = self.package_names[
                : self.package_limit - offset_p_limit
            ]
    ...
    def get_dlstats(self):
        ...
        if os.path.exists(os.path.join(self.data_folder, self.dlstats_folder)):
            file_list = glob.glob(
                os.path.join(self.data_folder, self.dlstats_folder, "*_*.json.gz")
            )
            if file_list:
                current_offset = (
                    max(
                        [
                            int(os.path.basename(r)[: -len(".json.gz")].split("_")[1])
                            for r in file_list
                        ]
                    )
                    + 1
        ...
        while current_offset < len(self.package_names):
            ...
            while True:
                url = (
                    self.dlstats_url
                    + f"daily/{start_date_txt}:{end_date_txt}/"
                    + ",".join(
                        self.package_names[current_offset : current_offset + batch_size]
                    )
                )
                filename = f"{current_offset}_{min(current_offset+batch_size-1,len(self.package_names))}.json.gz"
                dlstats_file = os.path.join(
                    self.data_folder, self.dlstats_folder, filename
                )
                self.logger.info(
                    f"""Current query for R download stats: {current_offset}-{current_offset+batch_size-1}/{len(self.package_names)}"""
                )
    ...
    def parse_dlstats(self):
        def gen_dl_info():
            for filename in sorted(
                glob.glob(
                    os.path.join(self.data_folder, self.dlstats_folder, "*.json.gz")
                )
            ):
    ...
    def parse_versions(self, p):
        ...
        return [
            (pid, version, c_at)
            for version, c_at in p["timeline"].items()
            if version != "archived"
        ]
    ...
    def get_R_versions(self):
        ...
        req = s.request(
            method="PROPFIND",
            url="https://svn.r-project.org/R/tags/",
            headers={"Depth": "1"},
        ).text
        del s
        ...
        for vr in versions_raw:
            ...
            if v[0] in "0123456789":
                raw_date = vr.split("<lp1:creationdate>")[1].split(
                    "</lp1:creationdate>"
                )[0]
            ...
            if version != "archived":
                if "Depends" in v_info.keys():
                    ans += [
                        ...
                        for dep, dep_semver in v_info["Depends"].items()
                    ]
                if "Imports" in v_info.keys():
                    ans += [
                        (pid, version, dep, dep_semver)
                        for dep, dep_semver in v_info["Imports"].items()
                    ]
        return ans
