...
class NPMFiller(generic.PackageFiller):
	...
	def parse_registry_info(self):
		...
		def registry_info(scope='default',step=self.log_step):
			...
			with gzip.open(registry_file,'rb') as f:
				iterator = ijson.items(io.BufferedReader(f,buffer_size=self.buffer_size),'rows.item')
				if self.package_limit is not None:
					iterator = itertools.islice(iterator,self.package_limit)
				for i,j in enumerate(iterator):
					if i%step == 0 and i!= 0:
						self.logger.info(f'NPM registry info for scope {scope}: {i} packages parsed')
					yield j
				
		self.package_list = (self.parse_package(p) for p in registry_info(scope='packages'))
		
		self.package_version_list = itertools.chain.from_iterable((self.parse_versions(p) for p in registry_info(scope='versions')))

		self.package_deps_list = itertools.chain.from_iterable((self.parse_deps(p) for p in registry_info(scope='dependencies')))
		...
		if 'repository' in p['doc'].keys() and len(p['doc']['repository']):
			if isinstance(p['doc']['repository'],str):
				...
				for u in p['doc']['repository']:
					if isinstance(u,str):
						return u
					elif 'type' in u.keys() and u['type'] == 'git' and 'url' in u.keys():
						return u['url']
					elif 'url' in u.keys():
						return u['url']
	...
	def parse_versions(self,p):
		...
		if 'time' in p['doc'].keys():
			return [(pid,version,c_at) for version,c_at in p['doc']['time'].items() if version not in ('created','modified','unpublished') ]
		else:
	...
	def parse_deps(self,p):
		...
		if 'versions' in p['doc'].keys():
			for version,v_info in p['doc']['versions'].items():
				if 'dependencies' in v_info.keys() and isinstance(v_info['dependencies'],dict):
					ans += [(pid,version,dep,dep_semver) for dep,dep_semver in v_info['dependencies'].items()]
