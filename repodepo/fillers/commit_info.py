...
class CommitsFiller(fillers.Filler):
    ...
    def list_commits(
        ...
        if not repo_obj.is_empty:
            ...
            def process_commit(commit):
                ...
                if basic_info_only:
                    ...
                    if commit.parents:
                        ...
                        )  # Inverted order wrt the expected one, to have expected values for insertions and deletions
            ...
            def subgenerator(walker_gen):
                for commit_sha in walker_gen:
                    ...
                    if (
                        ...
                        and (not allbranches)
                        and commit.commit_time < after_time
                    ):
                        break
                    if group_by == "authors":
                        if (
                            commit.author.email in tracked_args.keys()
                            and commit.committer.email in tracked_args.keys()
                        ):
                            continue
                        else:
                        ...
                        raise ValueError(
                            "Unrecognized group_by value: {}".format(group_by)
                        )
            ...
            if (
                basic_info_only
                or self.workers == 1
                or len(commit_sha_list) <= self.workers_commit_threshold
            ):
                ...
                def mp_generator(subgen):
                    ...
                    with mp.Pool(
                        1, initializer=gen_to_queue, initargs=(subgen, input_q)
                    ) as gen_pool:
                        with mp.Pool(
                            self.workers,
                            initializer=process,
    ...
    def get_repo(self, name, source, owner, engine="pygit2"):
        ...
        if not os.path.exists(repo_folder):
            ...
            if engine == "gitpython":
                ...
                raise ValueError(
                    "Engine not found for getting repo: {}\n Use gitpython or pygit2".format(
                        engine
                    )
                )
    ...
    def fill_authors(self, commit_info_list, repo_id, autocommit=True):
                ...
                (
                    {
                        ...
                        "committer_info": json.dumps({"name": c["committer_name"]}),
                    }
                    for c in tr_gen
                ),
            )
        ...
        if self.db.db_type == "postgres":
            self.db.cursor.execute(
                """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(%s,'identities',%s) ;""",
                (repo_id, latest_commit_time),
            )
        else:
            self.db.cursor.execute(
                """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(?,'identities',?) ;""",
                (repo_id, latest_commit_time),
            )
    ...
    def fill_commit_repos(self, commit_info_list, repo_id, autocommit=True):
        ...
        if self.db.db_type == "postgres":
            self.db.cursor.execute(
                """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(%s,'commit_repos',%s) ;""",
                (repo_id, latest_commit_time),
            )
        else:
            self.db.cursor.execute(
                """INSERT INTO table_updates(repo_id,table_name,latest_commit_time) VALUES(?,'commit_repos',?) ;""",
                (repo_id, latest_commit_time),
            )
    ...
    def fill_commit_parents(self, commit_info_list, repo_id, autocommit=True):
        ...
        if self.db.db_type == "postgres":
            ...
            self.db.cursor.execute(
                """UPDATE repositories SET latest_commit_time=COALESCE(%s,latest_commit_time) WHERE id=%s;""",
            ...
            self.db.cursor.execute(
                """UPDATE repositories SET latest_commit_time=COALESCE(?,latest_commit_time) WHERE id=?;""",
                (latest_commit_time, repo_id),
            )
    ...
    def fill_commit_orig_repo(self, only_null=True, force=False, autocommit=True):
        ...
        self.db.cursor.execute(
            """SELECT MAX(updated_at) FROM full_updates WHERE update_type='commits orig repos';"""
        )
        last_fu = self.db.cursor.fetchone()[0]
        self.db.cursor.execute(
            """SELECT MAX(updated_at) FROM full_updates WHERE update_type='commits';"""
        ...
        if not force and last_fu is not None and last_fu_commits <= last_fu:
            ...
            if only_null:
                ...
                self.logger.info(
                    "Filling commit origin repository attribution: update is_orig_repo to true for roots of fork trees"
                )
                self.db.cursor.execute(
                    """
                ...
                self.logger.info(
                    "Filling commit origin repository attribution: update is_orig_repo to true for repos that are the only ones owning the commit"
                )
                self.db.cursor.execute(
                    """
						UPDATE commit_repos SET is_orig_repo=true
                ...
                self.logger.info(
                    "Filling commit origin repository attribution: REDO update is_orig_repo to true for roots of fork trees"
                )
                self.db.cursor.execute(
                    """
						UPDATE commit_repos SET is_orig_repo=true
                ...
                self.logger.info(
                    "Filling commit origin repository attribution: REDO update is_orig_repo to true for repos that are the only ones owning the commit"
                )
                self.db.cursor.execute(
                    """
						UPDATE commit_repos SET is_orig_repo=true
                ...
                self.logger.info(
                    "Filling commit origin repository attribution: update is_orig_repo to false for repos elsewhere in fork trees"
                )
                self.db.cursor.execute(
                    """
                ...
                self.logger.info(
                    "Filling commit origin repository attribution: setting everything to null"
                )
                self.db.cursor.execute(
                    """
						UPDATE commit_repos SET is_orig_repo=NULL;"""
    ...
    def fill_commit_created_at(self, force=False, autocommit=True, batch_size=1000):
        ...
        self.db.cursor.execute(
            """SELECT MAX(updated_at) FROM full_updates WHERE update_type='commits created_at';"""
        )
        last_fu = self.db.cursor.fetchone()[0]
        self.db.cursor.execute(
            """SELECT MAX(updated_at) FROM full_updates WHERE update_type='commits';"""
        ...
        if not force and last_fu is not None and last_fu_commits <= last_fu:
            ...
            self.logger.info(
                "Fixed created_at field for {} commits (using current time)".format(
                    updated_count_currenttime
                )
            )
            ...
            self.logger.info(
                "Identified {} conflicting commit pairs concerning the created_at field (batch_size {})".format(
                    self.db.cursor.fetchone()[0], batch_size
                )
            )
            while updated_count != 0:
                ...
                self.logger.info(
                    "Identified {} conflicting commit pairs concerning the created_at field at step {} (batch_size {})".format(
                        conflicting_pairs, step, batch_size
                    )
                )
                ...
                self.db.cursor.execute(
                    "SELECT MAX(as_child),MAX(as_parent) FROM temp_conflict_levels;"
                )
                self.logger.info(
                    "Max depth of conflict in batch: {} as child, {} as parent".format(
                ...
                self.logger.info(
                    "Fixed created_at field for {} commits on step {}".format(
                        updated_count, step
                    )
                )
                step += 1
                ...
                self.logger.info(
                    "Remaining overall {} conflicting commit pairs concerning the created_at field (batch_size {})".format(
                        self.db.cursor.fetchone()[0], batch_size
                    )
                )
