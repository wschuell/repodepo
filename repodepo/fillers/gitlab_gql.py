...
class RequesterGitlab(github_gql.Requester):
    ...
    def __init__(self, url_root="gitlab.com", auth_header_prefix="Bearer ", **kwargs):
        ...
        github_gql.Requester.__init__(
            self, url=url, auth_header_prefix=auth_header_prefix, **kwargs
        )
...
class GitlabGQLFiller(github_gql.GHGQLFiller):
    ...
    def __init__(
        self,
        env_apikey="GITLAB_API_KEY",
        source_name="Gitlab",
        target_identity_type="gitlab_login",
        api_keys_file="gitlab_api_keys.txt",
        **kwargs,
    ):
        github_gql.GHGQLFiller.__init__(
            self,
            requester_class=RequesterGitlab,
            source_name=source_name,
            env_apikey=env_apikey,
...
class LoginsFiller(GitlabGQLFiller):
    ...
    def parse_query_result(self, query_result, identity_id, **kwargs):
        ...
        if query_result["repository"] is None:
            ...
            ans["repo_name"] = query_result["repository"]["fullPath"].split("/")[0]
            if (
                query_result["repository"]["repository"]["tree"]["lastCommit"]["author"]
                is None
            ):
                ans["login"] = None
                ans["created_at"] = None
            elif (
                query_result["repository"]["repository"]["tree"]["lastCommit"][
                    "author"
                ]["username"]
                is None
            ):
                ...
                ans["login"] = query_result["repository"]["repository"]["tree"][
                    "lastCommit"
                ]["author"]["username"]
                ans["created_at"] = None
            ans["commit_sha"] = query_result["repository"]["repository"]["tree"][
                "lastCommit"
            ]["id"]
    ...
    def insert_items(self, items_list, commit=True, db=None):
        github_gql.LoginsGQLFiller.insert_items(
            self, items_list=items_list, commit=commit, db=db
        )
...
class RepoCreatedAtFiller(GitlabGQLFiller):
    ...
    def parse_query_result(
        ...
        **kwargs,
        ...
        if repo_owner is None:
            ...
            repo_name = "/".join(
                query_result["repository"]["nameWithOwner"].split("/")[1:]
            )
    ...
    def insert_items(self, items_list, commit=True, db=None):
        github_gql.RepoCreatedAtGQLFiller.insert_items(
            self, items_list=items_list, commit=commit, db=db
        )
...
class IssueCommentsGQLFiller(github_gql.IssueCommentsGQLFiller):
    ...
    def insert_items(self, db=None, **kwargs):
        ...
        if len(kwargs["items_list"]):
            db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT 'issue_comments' ;"""
            )
...
class CompletePullRequestsGQLFiller(github_gql.CompletePullRequestsGQLFiller):
    ...
    def after_insert(self):
        if self.complete_info:
            ...
            self.db.add_filler(
                PullRequestCommentsGQLFiller(**self.get_generic_kwargs())
            )
        ...
        for e in query_result["repository"]["pullRequests"]["nodes"]:
            ...
            try:
                ...
                if e["state"] == "closed":
                    ...
                    raise ValueError(
                        f'''Unknown state for gitlab mergeRequest: {e['state']} , should be "opened", "merged" or "closed"'''
                    )
...
class PullRequestCommentsGQLFiller(github_gql.PRCommentsGQLFiller):
    ...
    def insert_items(self, db=None, **kwargs):
        ...
        if len(kwargs["items_list"]):
            db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT 'pullrequest_comments' ;"""
            )
