...
class DummyMetaFiller(fillers.Filler):
        ...
        **kwargs
        ...
        if packages_file is None:
            self.packages_file = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(fillers.__file__))),
                "tests",
                "testmodule",
                "dummy_data",
                "packages.csv",
    ...
    def prepare(self):
        ...
        self.db.add_filler(
            github_gql.UserCreatedAtGQLFiller(
                fail_on_wait=self.fail_on_wait,
                workers=self.workers,
                data_folder=data_folder,
                api_keys_file=api_keys_file,
        ...
        self.db.add_filler(
            github_gql.RepoCreatedAtGQLFiller(
                fail_on_wait=self.fail_on_wait,
                workers=self.workers,
                data_folder=data_folder,
                api_keys_file=api_keys_file,
        ...
        self.db.add_filler(
            github_gql.RepoLanguagesGQLFiller(
                fail_on_wait=self.fail_on_wait,
                workers=self.workers,
                data_folder=data_folder,
                api_keys_file=api_keys_file,
        ...
        self.db.add_filler(
            github_gql.UserLanguagesGQLFiller(
                fail_on_wait=self.fail_on_wait,
                workers=self.workers,
                data_folder=data_folder,
                api_keys_file=api_keys_file,
...
class MetaBotFiller(fillers.Filler):
    def prepare(self):
        ...
        # emails
        self.db.add_filler(
            bot_fillers.BotFiller(pattern="dependabot-%", identity_type="email")
        )
        self.db.add_filler(
            bot_fillers.BotListFiller(bot_list=[""], identity_type="email")
        )  # Marking as discarded empty email string in commits
        for prefix in ["", "%-", "%.", "%+"]:
        ...
        self.db.add_filler(
            bot_fillers.MGBotFiller(data_folder=self.data_folder)
        )  # Golzadeh et al. paper

        self.db.add_filler(
            bot_fillers.BotUserFiller()
        )  # propagating from identities to users and connected identities
