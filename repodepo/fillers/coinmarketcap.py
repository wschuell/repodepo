...
class CMCRequester(fillers.Filler):
    def __init__(
        ...
        env_apikey="COINMARKETCAP_API_KEY",
        api_keys_file="coinmarketcap_api_keys.txt",
        api_keys=None,
        **kwargs,
    ):
        if api_keys is None:
...
class CMCFiller(generic.PackageFiller):
        ...
        with open(os.path.join(self.data_folder, self.filename), "w") as f:
            ...
            for a in ans:
                if a["id"] not in past_ids:
                    f.write(
                        f""""{a['url']}","{a['name']}","{a['symbol']}","{a['slug']}","{a['date_added']}","{a['id']}"\n"""
                    )
                    past_ids.add(a["id"])
    ...
    def get_package_list(self):
        ...
        while True:
            ...
            ans += [
                {
                    "url": (
                        v["urls"]["source_code"][0]
                        if len(v["urls"]["source_code"])
                        else None
                    ),
                    "name": v["name"],
                    "symbol": v["symbol"],
                ...
                for k, v in currencies["data"].items()
            ]
            init_range += self.range_len
            self.save_file(ans)
            self.logger.info(f"""Retrieved currency info up to ID {ans[-1]['id']}""")
            if len(currencies["data"]) == 0 or (
                self.package_limit is not None and len(ans) >= self.package_limit
            ):
                break
        ...
        self.package_list = [
            (
                ...
                p["slug"],
                p["date_added"],
                (None if p["url"] in ("None", "") else p["url"]),
                None,
            )
            for p in ans
        ]
    ...
    def request_currencies(self, query_range):
        ...
        ans = self.requester.request(
            path="/v2/cryptocurrency/info",
            params={
                "aux": "urls,date_added",
                "id": ",".join([str(i) for i in query_range]),
            },
        )
        if ans["status"]["error_code"] == 400 and ans["status"][
            "error_message"
