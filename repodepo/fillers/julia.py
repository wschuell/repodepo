    ...
    if version == pattern:
        ...
        if "-" not in pattern:
            ...
            if len(parts) != 2:
                ...
                min_pattern = parts[0]
                max_pattern = parts[1]
        min_cond = semantic_version.Version(version) in semantic_version.SimpleSpec(
            f">={min_pattern}"
        )
        max_cond = semantic_version.Version(version) in semantic_version.SimpleSpec(
            f"<={max_pattern}"
        )
...
class JuliaHubFiller(generic.PackageFiller):
    ...
    def prepare(self):
        ...
        self.download(
            url=self.file_url,
            destination=os.path.join(self.data_folder, "juliahub_packages.json"),
        )
        with open(os.path.join(self.data_folder, "juliahub_packages.json"), "r") as f:
            packages_json = json.load(f)
        self.package_list = [
            (i, p["name"], None, p["metadata"]["repo"], None)
            for i, p in enumerate(packages_json["packages"])
        ]
        if not self.force:
...
class JuliaGeneralFiller(generic.PackageFiller):
    ...
    def get_downloadstats_overlap(self):
        ...
        intervals = [
            [
                ...
                datetime.datetime.strptime(max_db, "%Y-%m-%d"),
            ]
        ]
        for statsfile in glob.glob(
            os.path.join(self.data_folder, "julia_pkg_dlstats", "*.csv")
        ):
            df = pd.read_csv(statsfile)
            intervals.append(
                [
                    datetime.datetime.strptime(df["date"].min(), "%Y-%m-%d"),
                    datetime.datetime.strptime(df["date"].max(), "%Y-%m-%d"),
                ]
            )
    ...
    def prepare(self):
        ...
        self.db.cursor.execute(
            """SELECT 1 FROM full_updates WHERE update_type='julia_packages_general' LIMIT 1;"""
        )
        ans = list(self.db.cursor.fetchall())
        if len(ans) > 0 and not self.force:
            self.done = True
            self.logger.info(
                "Julia packages info from General Registry already present in DB, skipping"
            )
            return
        ...
        self.clone_repo(
            ...
            replace=self.replace_repo,
        )

        self.download(
            url=self.genie_pkgs_url,
            destination=os.path.join(self.data_folder, "pkgs_genie.db"),
        )

        self.download(
            url=self.dlstats_url,
            destination=os.path.join(
                self.data_folder,
                "julia_pkg_dlstats",
                f"""package_requests_by_date_{datetime.datetime.now().strftime('%Y_%m_%d')}.csv.gz""",
            ),
        )
        for gzfile in glob.glob(
            os.path.join(self.data_folder, "julia_pkg_dlstats", "*.gz")
        ):
        ...
        for statsfile in glob.glob(
            ...
            df = pd.read_csv(statsfile)
            df["version_str"] = None
            self.package_version_download_list += df[
                ["package_uuid", "version_str", "request_count", "date"]
            ].values.tolist()
        ...
        for p in glob.glob(os.path.join(data_folder, self.repo_folder, "?", "*")):
            ...
            elt = (
                content["uuid"],
                content["name"],
                self.package_dates[content["name"]],
                content["repo"],
            )
        ...
        for p in glob.glob(os.path.join(data_folder, self.repo_folder, "?", "*")):
            ...
            for version, version_info in content_v.items():
                elt = (
                    content_p["uuid"],
                    version,
                    self.version_dates[content_p["name"]][version],
                )
        ...
        for p, v_deps in self.deps_content.items():
            for v, deps in v_deps.items():
                for d, dv in deps.items():
                    self.package_deps_list.append(
                        (p, v, d, dv)
                    )  ## p and v are uuid not package name
    ...
    def get_versions_list(self):
        ...
        if not hasattr(self, "versions_list"):
            self.versions_list = []
            for p in glob.glob(
                os.path.join(self.data_folder, self.repo_folder, "?", "*")
            ):
                content_p = toml.load(os.path.join(p, "Package.toml"))
                content_v = toml.load(os.path.join(p, "Versions.toml"))
                for version, version_info in content_v.items():
    ...
    def save_dates(self):
        ...
        for p, v_l in self.version_dates.items():
            for v, d in v_l.items():
                to_write[(p, v)] = d
        to_write_lines = [
            f'''"{k[0]}","{k[1]}","{d.strftime('%Y-%m-%d %H:%M:%S')}"'''
            for k, d in to_write.items()
        ]
    ...
    def load_dates(self):
        ...
        if os.path.exists(dates_filepath):
            with open(dates_filepath, "r") as f:
                reader = csv.reader(f)
                return [
                    (p, v, datetime.datetime.strptime(d, "%Y-%m-%d %H:%M:%S"))
                    for p, v, d in reader
                ]
        else:
            return []
    ...
    def get_dates(self):
        ...
        self.logger.info(
            f"{count_versions}/{total_versions} version date info after parsing from file"
        )
        if count_versions != total_versions:
            ...
            for c in repo.walk(repo.head.target):
                if c.message.startswith("New package: ") or c.message.startswith(
                    "New version: "
                ):
                    ...
                    if (
                        package_name in versions_dict.keys()
                        and version in versions_dict[package_name]
                    ):
                        try:
            ...
            self.logger.info(
                f"{count_not_parseable} non parseable commits for getting version date from commit message"
            )

            count_versions = sum([len(v.keys()) for v in self.version_dates.values()])
            self.logger.info(
                f"{count_versions}/{total_versions} version date info after parsing from commit messages"
            )
            """
            ...
            try:
                for p, pname, v in sorted(self.versions_list):
                    if (
                        pname in self.version_dates.keys()
                        and v in self.version_dates[pname].keys()
                    ):
                        continue
                    else:
                        count += 1
                        ...
                        self.logger.info(
                            f"Getting version date for {rel_path} for version {v} ({count}/{missing_versions})"
                        )
                        cmd = [
                            "git",
                            "-C",
                            ...
                            f"""["{v}"]""",
                            "--",
                            os.path.join(rel_path, "Versions.toml"),
                        ]  # ,'|','cat']
                        ...
                        self.version_dates[pname][v] = datetime.datetime.fromtimestamp(
                            repo.get(oid).commit_time
                        )
            finally:
                self.save_dates()
    ...
    def parse_deps(self):
        ...
        if not hasattr(self, "deps_content"):
            self.deps_content = {}
            p_list = sorted(
                glob.glob(os.path.join(self.data_folder, self.repo_folder, "?", "*"))
            )
            for i, p in enumerate(p_list):
                ...
                if content_p["name"] == "julia":
                    ...
                    self.logger.info(
                        f"""Parsing dependencies for package {content_p['name']} ({i+1}/{len(p_list)})"""
                    )
                content_v = toml.load(os.path.join(p, "Versions.toml"))
    ...
    def apply(self):
        ...
        self.db.cursor.execute(
            """INSERT INTO full_updates(update_type) SELECT 'julia_packages_general';"""
        )
        self.db.connection.commit()
