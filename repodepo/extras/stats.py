...
class Stats(generic_getters.Getter):
    ...
    def __init__(self, autosave_tmp=False, tmp_filepath=None, **kwargs):
        ...
        if tmp_filepath is None:
            self.tmp_filepath = os.path.join(
                "tmp", self.__class__.__name__ + "_tmp.yml"
            )
        else:
...
class DBStats(Stats):
    ...
    def get_result(self, db=None, **kwargs):
        ...
        if db is None:
            ...
            raise NotImplementedError(
                "For DBStats and children classes, call to another DB is not implemented/passed down to .get()"
            )
        if db is None:
    ...
    def get_nb_withrepo(self, cloned=False):
        ...
        if cloned:
            self.db.cursor.execute(
                """SELECT COUNT(p.repo_id) AS cnt,'_all' AS sname FROM packages p
            ...
            self.db.cursor.execute(
                """SELECT COUNT(DISTINCT p.repo_id) AS cnt,'_all' AS sname FROM packages p
									UNION
...
class URLStats(DBStats):
    ...
    def get_nb_urls(self):
        self.db.cursor.execute(
            "SELECT COUNT(DISTINCT COALESCE(cleaned_url,id)) FROM urls;"
        )
        return self.db.cursor.fetchone()[0]
    ...
    def get_nb_withrepo(self, cloned=False):
        ...
        if cloned:
            self.db.cursor.execute(
                """SELECT COUNT(*) AS cnt,'_all' AS sname FROM urls u
									INNER JOIN urls cu
            ...
            self.db.cursor.execute(
                """SELECT COUNT(DISTINCT COALESCE(u.cleaned_url,u.id)) AS cnt,'_all' AS sname FROM urls u
									INNER JOIN urls cu
									ON cu.id=COALESCE(u.cleaned_url,u.id)
									INNER JOIN repositories r
									ON cu.id=r.url_id AND r.cloned
...
class RepoStats(DBStats):
    ...
    def get_merged(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT '_all' AS sname,COUNT(*) AS cnt FROM merged_repositories
    ...
    def get_nb_repos(self, cloned=False):
        if cloned:
            self.db.cursor.execute(
                """SELECT COUNT(*) AS cnt,'_all' AS sname FROM repositories r WHERE r.cloned
				UNION
            ...
            self.db.cursor.execute(
                """SELECT COUNT(*) AS cnt,'_all' AS sname FROM repositories r
				UNION
...
class CommitsStats(DBStats):
    ...
    def get(self, db, **kwargs):
        ...
        results["with_forks"]["nb_total"]["no_bots"] = self.get_nb_commits_forks(
            onlybots=False
        )
        results["with_forks"]["nb_total"]["only_bots"] = self.get_nb_commits_forks(
            onlybots=True
        )
        results["with_forks"]["max"] = OrderedDict()
        results["with_forks"]["max"]["no_bots"] = self.get_nb_commits_forks(
            onlybots=False, option="max"
        )
        results["with_forks"]["max"]["only_bots"] = self.get_nb_commits_forks(
            onlybots=True, option="max"
        )
        return results
    ...
    def get_nb_commits_forks(self, onlybots=False, option=None):
        self.db.cursor.execute(
            "CREATE TEMPORARY TABLE IF NOT EXISTS commit_repos_multiplicity(commit_id BIGINT PRIMARY KEY,fork_count BIGINT);"
        )
        self.db.cursor.execute(
            "SELECT commit_id FROM commit_repos_multiplicity LIMIT 1;"
        )
...
class DepsStats(DBStats):
    ...
    def get(self, db, **kwargs):
        ...
        if not self.only_filtered:
            ...
            results["packagespace"]["cycles"] = self.get_cycles(
                space="p",
                filtered=False,
                detailed=self.detailed,
                network=self.network_p,
            )

        results["packagespace_filtered"] = OrderedDict()
        results["packagespace_filtered"]["nb_links"] = len(
            self.network_p_filtered.edges
        )
        if not self.only_filtered:
            results["packagespace_filtered"]["nb_links_filtered"] = len(
                self.network_p.edges
            ) - len(self.network_p_filtered.edges)
        results["packagespace_filtered"]["cycles"] = self.get_cycles(
            space="p",
            filtered=True,
            detailed=self.detailed,
            network=self.network_p_filtered,
        )

        if not self.only_filtered:
            results["packagespace_timestamp"] = OrderedDict()
            results["packagespace_timestamp"]["nb_links"] = len(
                self.network_p_timestamp.edges
            )
            results["packagespace_timestamp"]["cycles"] = self.get_cycles(
                space="p",
                filtered=False,
                detailed=self.detailed,
                network=self.network_p_timestamp,
            )

        results["packagespace_timestamp_filtered"] = OrderedDict()
        results["packagespace_timestamp_filtered"]["nb_links"] = len(
            self.network_p_filtered_timestamp.edges
        )
        if not self.only_filtered:
            results["packagespace_timestamp_filtered"]["nb_links_filtered"] = len(
                self.network_p_timestamp.edges
            ) - len(self.network_p_filtered_timestamp.edges)
        results["packagespace_timestamp_filtered"]["cycles"] = self.get_cycles(
            space="p",
            filtered=True,
            detailed=self.detailed,
            network=self.network_p_filtered_timestamp,
        )

        if not self.only_filtered:
            results["repospace"] = OrderedDict()
            results["repospace"]["nb_links"] = len(self.network_r.edges)
            results["repospace"]["cycles"] = self.get_cycles(
                space="r",
                filtered=False,
                detailed=self.detailed,
                network=self.network_r,
            )

        results["repospace_filtered"] = OrderedDict()
        results["repospace_filtered"]["nb_links"] = len(self.network_r_filtered.edges)
        if not self.only_filtered:
            results["repospace_filtered"]["nb_links_filtered"] = len(
                self.network_r.edges
            ) - len(self.network_r_filtered.edges)
        results["repospace_filtered"]["cycles"] = self.get_cycles(
            space="r",
            filtered=True,
            detailed=self.detailed,
            network=self.network_r_filtered,
        )

        if not self.only_filtered:
            results["repospace_timestamp"] = OrderedDict()
            results["repospace_timestamp"]["nb_links"] = len(
                self.network_r_timestamp.edges
            )
            results["repospace_timestamp"]["cycles"] = self.get_cycles(
                space="r",
                filtered=False,
                detailed=self.detailed,
                network=self.network_r_timestamp,
            )

        results["repospace_timestamp_filtered"] = OrderedDict()
        results["repospace_timestamp_filtered"]["nb_links"] = len(
            self.network_r_filtered_timestamp.edges
        )
        if not self.only_filtered:
            results["repospace_timestamp_filtered"]["nb_links_filtered"] = len(
                self.network_r_timestamp.edges
            ) - len(self.network_r_filtered_timestamp.edges)
        results["repospace_timestamp_filtered"]["cycles"] = self.get_cycles(
            space="r",
            filtered=True,
            detailed=self.detailed,
            network=self.network_r_filtered_timestamp,
        )
    ...
    def set_network_timestamp(self, timestamp=None):
        ...
        if not hasattr(self, "network_r_timestamp"):
            mat = edge_getters.RepoToRepoDeps(
                db=self.db, ref_time=timestamp, filter_deps=False
            ).get_result(raw_result=True)
            ...
            self.network_r_timestamp.add_edges_from(
                ((r["repo_id"], r["dep_id"]) for r in mat)
            )
    ...
    def set_network_filtered_timestamp(self, timestamp=None):
        ...
        if not hasattr(self, "network_r_filtered_timestamp"):
            mat = edge_getters.RepoToRepoDeps(
                db=self.db, ref_time=timestamp, filter_deps=True
            ).get_result(raw_result=True)
            ...
            self.network_r_filtered_timestamp.add_edges_from(
                ((r["repo_id"], r["dep_id"]) for r in mat)
            )
    ...
    def get_cycles(self, network, filtered=True, space="r", detailed=None):
        ...
        elts = dict()
        links = dict()
        rk_direct, rk_indirect, rk_names = rank_getters.RepoRankNameGetter(
            db=self.db
        ).get_result()

        self.db.cursor.execute(
            "SELECT p.id,s.name,p.name FROM packages p INNER JOIN sources s ON s.id=p.source_id;"
        )

        package_names = {
            int(p_id): "/".join([s, pname])
            for p_id, s, pname in self.db.cursor.fetchall()
        }
        ...
        for i, c in enumerate(scyc):
            if self.limit is not None and i >= self.limit:
                self.logger.warning(
                    "Stopping cycle detection after {} cycles".format(i)
                )
        ...
        if detailed:
            ...
            ans["elements_involved"] = sorted(
                [{get_name(e): v} for e, v in elts.items()],
                key=lambda x: -list(x.values())[0],
            )[:10]
            links_involved = sorted(
                [{(get_name(e[0]), get_name(e[1])): v} for e, v in links.items()],
                key=lambda x: -list(x.values())[0],
            )[:10]
            ans["links_involved"] = [
                {str(dk): dv for dk, dv in d.items()} for d in links_involved
            ]
            if space == "r":
                ...
                for main_lk in main_lk_l[:5]:
                    ...
                    ans["packagespace_links"][
                        str(main_lk)
                    ] = self.get_packagespace_edges(
                        source_s=source_s,
                        repo_s=repo_s,
                        source_d=source_d,
                        repo_d=repo_d,
    ...
    def get_packagespace_edges(self, source_s, repo_s, source_d, repo_d):
        ...
        if self.db.db_type == "postgres":
            self.db.cursor.execute(
                ...
                {
                    "source_s": source_s,
                    "rowner_s": repo_owner_s,
                    "rname_s": repo_name_s,
                    "source_d": source_d,
                    "rowner_d": repo_owner_d,
                    "rname_d": repo_name_d,
                },
            )
        else:
...
class IssuesStats(DBStats):
    ...
    def get_nb_total(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM issues i
				UNION
    ...
    def get_nb_closed(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM issues i WHERE i.closed_at IS NOT NULL
				UNION
					SELECT * FROM (SELECT COUNT(*),s.name AS sname FROM issues i
					INNER JOIN repositories r
    ...
    def get_labels(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(DISTINCT l.label) AS cnt,'_all' AS sname FROM issue_labels l 
				UNION
    ...
    def get_comments(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM issue_comments l 
				UNION
    ...
    def get_reactions(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM issue_reactions l 
				UNION
    ...
    def get_comment_reactions(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM issue_comment_reactions l 
				UNION
...
class PRsStats(DBStats):
    ...
    def get_nb_total(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM pullrequests i
				UNION
    ...
    def get_nb_closed(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM pullrequests i WHERE i.closed_at IS NOT NULL
				UNION
					SELECT * FROM (SELECT COUNT(*),s.name AS sname FROM pullrequests i
					INNER JOIN repositories r
					ON r.id=i.repo_id AND i.closed_at IS NOT NULL
    ...
    def get_nb_merged(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM pullrequests i WHERE i.merged_at IS NOT NULL
				UNION
					SELECT * FROM (SELECT COUNT(*),s.name AS sname FROM pullrequests i
					INNER JOIN repositories r
    ...
    def get_labels(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(DISTINCT l.label) AS cnt,'_all' AS sname FROM pullrequest_labels l 
				UNION
    ...
    def get_comments(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM pullrequest_comments l 
				UNION
    ...
    def get_reactions(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM pullrequest_reactions l 
				UNION
    ...
    def get_comment_reactions(self):
        ans = OrderedDict()
        self.db.cursor.execute(
            """SELECT COUNT(*) AS cnt,'_all' AS sname FROM pullrequest_comment_reactions l 
				UNION
