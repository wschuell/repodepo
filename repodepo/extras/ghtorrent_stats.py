...
class GHTorrentStats(DBStats):
    ...
    def fill_temp_commits(self):
        ...
        self.ght_cur.execute(
            """CREATE TEMP TABLE IF NOT EXISTS temp_repo_tools_commits(
    ...
    def fill_temp_users(self):
        ...
        """
        self.ght_cur.execute(
            """CREATE TEMP TABLE IF NOT EXISTS temp_repo_tools_users(
        ...
        if len(list(self.ght_cur.fetchall())) == 0:
            ...
            self.db.cursor.execute(
                """SELECT DISTINCT i.identity FROM identities i
    ...
    def fill_temp_repos(self):
        ...
        """
        self.ght_cur.execute(
            """CREATE TEMP TABLE IF NOT EXISTS temp_repo_tools_repos(
							owner TEXT,
							name TEXT,
							cloned BOOLEAN,
        ...
        if len(list(self.ght_cur.fetchall())) == 0:
            ...
            self.db.cursor.execute(
                """SELECT DISTINCT r.owner,r.name,r.cloned FROM repositories r
				INNER JOIN sources s
    ...
    def fill_temp_userrepos(self):
        ...
        """
        self.ght_cur.execute(
            """CREATE TEMP TABLE IF NOT EXISTS temp_repo_tools_userrepos(
							owner TEXT,
							name TEXT,
							login TEXT,
        ...
        if len(list(self.ght_cur.fetchall())) == 0:
            ...
            # 	;''',{'max_time':self.max_time})
            self.db.cursor.execute(
                """SELECT DISTINCT r.owner,r.name,i2.identity FROM repositories r
				INNER JOIN sources s
				ON s.name='GitHub' AND s.id=r.source
				INNER JOIN commit_repos cr
				ON cr.repo_id=r.id
            ...
            self.ght_cur.execute(
                "CREATE INDEX IF NOT EXISTS userrepo_uidx ON temp_repo_tools_userrepos(login);"
            )
    ...
    def fill_temp_userrepos_ght(self):
        ...
        self.ght_cur.execute(
            """CREATE TEMP TABLE IF NOT EXISTS temp_repo_tools_userrepos_ght(
							owner TEXT,
							name TEXT,
							login TEXT,
							PRIMARY KEY(owner,name,login)
        ...
        if len(list(self.ght_cur.fetchall())) == 0:
            ...
            self.ght_cur.execute(
                "CREATE INDEX IF NOT EXISTS userrepo_ght_uidx ON temp_repo_tools_userrepos_ght(login);"
            )
    ...
    def fill_temp_stars(self):
        ...
        """
        self.ght_cur.execute(
            """CREATE TEMP TABLE IF NOT EXISTS temp_repo_tools_stars(
							starrer TEXT,
							owner TEXT,
							name TEXT,
        ...
        if len(list(self.ght_cur.fetchall())) == 0:
            ...
            self.db.cursor.execute(
                """SELECT DISTINCT s.login,r.owner,r.name FROM stars s
				INNER JOIN repositories r
				ON s.repo_id=r.id
    ...
    def fill_temp_forks(self):
        ...
        """
        self.ght_cur.execute(
            """CREATE TEMP TABLE IF NOT EXISTS temp_repo_tools_forks(
							forked_owner TEXT,
							forked_name TEXT,
							forking_owner TEXT,
							forking_name TEXT,
        ...
        if len(list(self.ght_cur.fetchall())) == 0:
            ...
            self.db.cursor.execute(
                """SELECT DISTINCT f.forking_repo_url,r.owner,r.name FROM forks f
				INNER JOIN repositories r
                ...
                (
                    {
                        "forked_owner": r[1],
                        "forked_name": r[2],
                        "forking_owner": r[0][11:].split("/")[0],
                        "forking_name": r[0][11:].split("/")[1],
                    }
                    for r in results
                ),
            )
...
class UserGHTStats(GHTorrentStats):
    def get(self, **kwargs):
        ...
        self.ght_cur.execute(
            """SELECT COUNT(DISTINCT u2.id),COUNT(DISTINCT tu.login) FROM temp_repo_tools_repos tr
			INNER JOIN projects r
			ON tr.name=r.name
			INNER JOIN users u
			ON r.owner_id=u.id
        ...
        self.ght_cur.execute(
            """SELECT COUNT(*) FROM temp_repo_tools_users tu
			INNER JOIN users u
			ON tu.login=u.login
			;"""
        )
        results["globalusers_common"] = self.ght_cur.fetchone()[0]
        results["globalusers_missing_ght"] = (
            results["users_rust"] - results["users_common"]
        )
...
class CommitsGHTStats(GHTorrentStats):
    def get(self, **kwargs):
        ...
        self.ght_cur.execute(
            """SELECT COUNT(DISTINCT c.id),COUNT(DISTINCT tc.sha) FROM temp_repo_tools_repos tr
			INNER JOIN projects r
			ON tr.name=r.name
			INNER JOIN users u
			ON r.owner_id=u.id
        ...
        results["commits_missing_ght"] = (
            results["commits_rust"] - results["commits_common"]
        )

        self.ght_cur.execute(
            """SELECT COUNT(*) FROM temp_repo_tools_commits tc
			INNER JOIN commits c
			ON tc.sha=c.sha
			;"""
        )
        results["globalcommits_common"] = self.ght_cur.fetchone()[0]
        results["globalcommits_missing_ght"] = (
            results["commits_rust"] - results["globalcommits_common"]
        )
...
class ReposGHTStats(GHTorrentStats):
    def get(self, **kwargs):
        ...
        # 	;''')
        self.ght_cur.execute(
            """SELECT COUNT(*) FROM temp_repo_tools_repos tr
			INNER JOIN projects r
			ON tr.name=r.name
			INNER JOIN users u
			ON r.owner_id=u.id
...
class StarsGHTStats(GHTorrentStats):
    def get(self, **kwargs):
        ...
        self.ght_cur.execute(
            """SELECT COUNT(DISTINCT CONCAT(u2.id,'/',r.id)),COUNT(DISTINCT CASE WHEN ts.starrer IS NOT NULL THEN CONCAT(ts.starrer,'//',ts.owner,'/',ts.name) ELSE NULL END) FROM temp_repo_tools_repos tr
			INNER JOIN projects r
			ON tr.name=r.name
			INNER JOIN users u
			ON r.owner_id=u.id
        ...
        self.ght_cur.execute(
            """SELECT COUNT(*) FROM temp_repo_tools_stars ts
			INNER JOIN users u
			ON ts.owner=u.login
			INNER JOIN projects r
			ON ts.name=r.name
...
class ForksGHTStats(GHTorrentStats):
    def get(self, **kwargs):
        ...
        self.ght_cur.execute(
            """SELECT COUNT(DISTINCT CONCAT(f.id,'/',r.id)),COUNT(DISTINCT CASE WHEN tf.forked_owner IS NOT NULL THEN CONCAT(tf.forked_owner,'/',tf.forked_name,'//',tf.forking_owner,'/',tf.forking_name) ELSE NULL END) FROM temp_repo_tools_repos tr
			INNER JOIN projects r
			ON tr.name=r.name
			INNER JOIN users u
			ON r.owner_id=u.id
        ...
        self.ght_cur.execute(
            """SELECT COUNT(*) FROM temp_repo_tools_forks tf
			INNER JOIN users u
...
class UserReposGHTStats(GHTorrentStats):
    def get(self, **kwargs):
        ...
        # 	;''')
        self.ght_cur.execute(
            """SELECT COUNT(DISTINCT CONCAT(turg.login,'//',turg.owner,'/',turg.name)),
				COUNT(DISTINCT CASE WHEN tur.login IS NOT NULL THEN CONCAT(tur.login,'//',tur.owner,'/',tur.name) ELSE NULL END)
        ...
        # 	;''')
        self.ght_cur.execute(
            """SELECT COUNT(DISTINCT CONCAT(tur.login,'//',tur.owner,'/',tur.name))
        ...
        results["userrepos_edges_missing_ght"] = (
            results["userrepos_edges_rust"] - results["userrepos_edges_common"]
        )
