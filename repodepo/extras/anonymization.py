...
def anonymize(db, salt=None, keep_email_suffixes=True, ignore_error=False):
    try:
        db.add_filler(
            AnonymizationMetaFiller(salt=salt, keep_email_suffixes=keep_email_suffixes)
        )
        db.fill_db()
    except errors.RepoToolsDBStructError:
        if ignore_error:
            db.logger.info(
                "Skipping anonymization step, DB seems to have already been cleaned (_dbinfo table missing)"
            )
        else:
            raise
...
def anonymize_emails(db, salt=None):
    db.add_filler(
        EmailAnonFiller(
            table="identities", field="identity", it_field="identity_type_id", salt=salt
        )
    )
    db.add_filler(
        EmailAnonFiller(
            table="users",
            field="creation_identity",
            it_field="creation_identity_type_id",
            salt=salt,
        )
    )
    db.fill_db()
...
class MergedIDAnonFiller(fillers.Filler):
    def apply(self):
        self.db.cursor.execute(
            """ UPDATE merged_identities SET reason='Parsed email as github_login' WHERE reason LIKE 'Parsed email % github_login%'; """
        )
        self.db.cursor.execute(
            """ UPDATE merged_identities SET reason='Parsed email as gitlab_login' WHERE reason LIKE 'Parsed email % gitlab_login%'; """
        )
...
class AnonymizationFiller(fillers.Filler):
    ...
    def set_salt(self):
        self.db.cursor.execute(
            """SELECT info_content FROM _dbinfo WHERE info_type='anonymization_salt'; """
        )
        ...
        if db_salt is None:
            ...
            if self.db.db_type == "postgres":
                self.db.cursor.execute(
                    """INSERT INTO _dbinfo(info_type,info_content) SELECT 'anonymization_salt',%(salt)s;""",
                    {"salt": self.salt},
                )
            else:
                self.db.cursor.execute(
                    """INSERT INTO _dbinfo(info_type,info_content) SELECT 'anonymization_salt',:salt;""",
                    {"salt": self.salt},
                )
            ...
            raise ValueError(
                "Trying to anonymize with a different salt than the one already used for previous anonymization. DB salt:{} filler salt:{}".format(
                    db_salt, self.salt
                )
            )
    ...
    def prepare(self):
        ...
        if not self.force:
            if self.db.db_type == "postgres":
                self.db.cursor.execute(
                    """SELECT COUNT(*) FROM full_updates WHERE update_type=%(update_name)s;""",
                    {"update_name": update_name},
                )
            else:
                self.db.cursor.execute(
                    """SELECT COUNT(*) FROM full_updates WHERE update_type=:update_name;""",
                    {"update_name": update_name},
                )
    ...
    def register_update(self, table=None, field=None, autocommit=True):
        ...
        if self.db.db_type == "postgres":
            self.db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT %(update_name)s;""",
                {"update_name": update_name},
            )
        else:
            self.db.cursor.execute(
                """INSERT INTO full_updates(update_type) SELECT :update_name;""",
                {"update_name": update_name},
            )
        if autocommit:
            self.db.connection.commit()
    ...
    def hash_field(self, id_field=None, field=None, table=None):
        ...
        if self.db.db_type == "postgres":
            ...
            query = """
						...
						FROM {table}
					;""".format(
                field=field, id_field=id_field, table=table
            )
            ...
            updated = [
                (iid, modify(old_id=old_val, salt=self.salt))
                for iid, old_val in to_update
            ]
            ...
            self.db.cursor.executemany(
                query,
                (
                    {"salt": self.salt, "iid": iid, "new_val": new_val}
                    for iid, new_val in updated
                ),
            )
            self.db.cursor.execute(
                """
        ...
        if self.db.db_type == "postgres":
            self.db.cursor.execute(
				...
				;"""
                    ...
                    table=psycopg2.sql.Identifier(table),
                    where_clause=psycopg2.sql.SQL(self.where_clause).format(
                        it_field=psycopg2.sql.Identifier(it_field)
                    ),
                ),
                {"salt": self.salt},
            ...
            query = """
                ...
                it_field=it_field,
                table=table,
                where_clause=self.where_clause.format(it_field=it_field),
            )
...
class EmptyTableFiller(AnonymizationFiller):
    ...
    def empty_table(self):
        if self.db.db_type == "postgres":
            self.db.cursor.execute(
                psycopg2.sql.SQL("DELETE FROM {table};").format(
                    table=psycopg2.sql.Identifier(self.table)
                )
            )
        else:
...
class NullifyFiller(AnonymizationFiller):
    ...
    def nullify(self, id_field=None, field=None, it_field=None, table=None):
        ...
        if self.db.db_type == "postgres":
            ...
            self.db.cursor.executemany(
                query, ({"salt": self.salt, "iid": iid} for iid, old_val in to_update)
            )
