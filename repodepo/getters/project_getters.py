    ...
    def clean_id(self, db, project_id):
        ...
        if isinstance(project_id, str):
            ...
            if len(project_id.split("/")) == 2:
                ...
                raise ValueError(
                    "Repository syntax not parsable: {}".format(projectname)
                )
        ...
        raise ValueError(
            "No such repository or unparsed syntax (not int, str or tuple): {}".format(
                project_id
            )
        )
        ...
        if project_id is not None:
            ...
            if aggregated:
                ...
                if time_window is None:
                    ...
                    df = pd.DataFrame(
                        data=query_result, columns=(self.measure_name, "project_id")
                    )

                    if self.is_date:
                        df[self.measure_name] = pd.to_datetime(df[self.measure_name])
                    project_ids = (
                        generic_getters.RepoIDs(db=db)
                    ...
                    df = pd.DataFrame(
                        data=query_result,
                        columns=(self.measure_name, "timestamp", "project_id"),
                    ).convert_dtypes()
...
class Forks(ProjectGetter):
    ...
    def query_notimeinfo(
        self, db, start_date, end_date, project_id=None, time_window=None
    ):
        if db.db_type == "postgres":
            db.cursor.execute(
                """
...
# issues
#########################
class Issues(ProjectGetter):
    measure_name = "issues"

    def __init__(self, closed_only=False, **kwargs):
        ProjectGetter.__init__(self, **kwargs)
...
class ClosedIssues(Issues):
    """
    Distinction between closed_only(still filtering by created_at) and closed!
    """

    measure_name = "issues_closed"
