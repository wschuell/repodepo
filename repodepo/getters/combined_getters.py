    ...
    def get_result(self):
        ...
        for sg in self.subgetters:
            ...
            df = pd.merge(
                ...
                sg_df,
                how="outer",
                left_on=["project_id", "timestamp"],
                right_on=["project_id", "timestamp"],
            )

        creation_dates = generic_getters.RepoCreatedAt(
            db=self.db, force_repo_date=self.force_repo_date
        ).get_result()
        ...
        if self.with_reponame:
            reponames = (
                generic_getters.RepoNames(db=self.db)
                .get_result()
...
class IdleReposGetter(UsageGetter):
    subgetters = [
        {
            "class": project_getters.LastCommit,
            "rename": {},
            "extra_kwargs": {"cumulative": False},
        },
        {
            "class": project_getters.Downloads,
            "rename": {"downloads": "downloads_noncumul"},
            "extra_kwargs": {"cumulative": False},
        },
        {
            "class": project_getters.Issues,
            "rename": {"issues": "issues_noncumul"},
            "extra_kwargs": {"cumulative": False},
        },
        {
            "class": project_getters.ClosedIssues,
            "rename": {"issues_closed": "issues_closed_noncumul"},
            "extra_kwargs": {"cumulative": False},
        },
        {
            "class": project_getters.PullRequests,
            "rename": {"pullrequests": "pullrequests_noncumul"},
            "extra_kwargs": {"cumulative": False},
        },
        {
            "class": project_getters.MergedPullRequests,
            "rename": {"pullrequests_merged": "pullrequests_merged_noncumul"},
            "extra_kwargs": {"cumulative": False},
        },
...
class DepsGetter(CombinedGetter):
    ...
    def get_result(self):
        date_range = pd.date_range(
            self.start_date, self.end_date, freq=pandas_freq[self.time_window]
        ).to_pydatetime()
        ...
        for t in date_range:
            self.logger.info(
                "Getting dependency network on {}".format(
                    datetime.datetime.strftime(t, "%Y-%m-%d")
                )
            )
            deps_df = pd.DataFrame(
                edge_getters.RepoToRepoDeps(db=self.db, ref_time=t).get_result(
                    raw_result=True
                ),
                columns=["repo_id", "dep_id"],
            )
        ...
        if self.with_reponame:
            ...
            depnames = reponames.rename(
                columns={"project_id": "dep_id", "repo_name": "dep_name"}
            ).set_index("dep_id")
            reponames = reponames.rename(columns={"project_id": "repo_id"}).set_index(
                "repo_id"
            )
...
class ContributionsGetter(CombinedGetter):
    ...
    def get_result(self):
        ...
        for t in date_range:
            self.logger.info(
                "Getting contribution network on {}".format(
                    datetime.datetime.strftime(t, "%Y-%m-%d")
                )
            )
            res = (
                ...
                for d in self.contrib_getter_class(
                    db=self.db,
                    start_time=t - relativedelta(**{self.time_window: 1}),
                    end_time=t,
                ).get_result(raw_result=True)
            )
            deps_df = pd.DataFrame(
                res, columns=["timestamp", "repo_id", "user_id", "nb_commits"]
            )
        ...
        order = ["repo_name", "user_login", "nb_commits"]
        if not self.with_reponame:
            order.remove("repo_name")
