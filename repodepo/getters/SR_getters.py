...
class SRGetter(Getter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if scenario_space in scenario_space_list:
            ...
            raise ValueError(
                f"Scenario space {scenario_space} not in available list: {scenario_space_list}"
            )
        ...
        if dl_correction:
            self.dl_getter = rank_getters.RepoDLCorrectionRank(
                db=self.db, start_time=self.start_time, end_time=self.end_time
            )  # ,dl_correction=dl_correction)
        else:
            self.dl_getter = rank_getters.RepoDLRank(
                db=self.db, start_time=self.start_time, end_time=self.end_time
            )  # ,dl_correction=dl_correction)
        self.deps_getter = edge_getters.RepoToRepoDeps(
            db=self.db, ref_time=self.end_time
        )
        self.devs_getter = edge_getters.DevToRepo(
            db=self.db, start_time=self.start_time, end_time=self.end_time
        )
    ...
    def set_vaccinated_repos(self, vaccinated_repos=None, vaccinated_repo_ranks=None):
        if vaccinated_repo_ranks is not None:
            ...
            raise SyntaxError(
                "Either vaccinated_repos or vaccinated_repo_ranks should be provided"
            )
    ...
    def get_dl_vec(self):
        if self.dl_weights:
            if not hasattr(self, "dl_vec"):
                dl_direct, dl_indirect, dl_values = self.dl_getter.get_result(
                    deps_mat=self.get_deps_mat(), orig_id_rank=True
                )
    ...
    def get(self, db, **kwargs):
        ...
        while repo_status_cond.nnz or (count == 0):
            ...
            repo_status_cond = self.propagate(
                dev_status=dev_status_cond,
                repo_status=repo_status_cond,
                force_dev_mat=True,
                info="cond",
            )
            ...
            if count >= self.iter_max:
                self.logger.info(
                    "Reached {} iterations, returning with current state".format(
                        self.iter_max
                    )
                )
                break
    ...
    def propagate(self, dev_status, repo_status, force_dev_mat=False, info=None):
        ...
        dev_contrib.data = 1.0 - dev_contrib.data
        deps_contrib.data = 1.0 - deps_contrib.data
        ...
        dev_contrib.data = 1.0 - dev_contrib.data
        dev_contrib.data[np.isnan(dev_contrib.data)] = 1.0

        deps_contrib.data = 1.0 - deps_contrib.data
        deps_contrib.data[np.isnan(deps_contrib.data)] = 1.0

        # multiply point wise
        repo_status_new = (
            dev_contrib + deps_contrib - dev_contrib.multiply(deps_contrib)
        )

        repo_status_new = self.postprocess_repostatus(
            repo_status=repo_status_new, info=info
        )
    ...
    def postprocess_repostatus(self, repo_status, info=None):
        if self.scenario_space != "repos" or info == "cond":
            ...
            repo_status.data[repo_status.data > 1] = 1.0
            return repo_status

    def get_dev_contrib(self, dev_status, force=False):
        if (
            self.scenario_space == "dev_contrib_vec" or not force
        ) and self.stationary_devcontrib:
    ...
    def parallelize_repo_space(self, factor):
        ...
        for i in range(factor - 1):
            new_deps_mat = scipy.sparse.bmat(
                [[self.deps_mat, None], [None, new_deps_mat]]
            )
        ...
        for i in range(factor - 1):
            new_init_repo_status = scipy.sparse.vstack(
                [new_init_repo_status, self.init_repo_status]
            )
...
class SRLeontief(SRGetter):
    def propagate(self, dev_status, repo_status, force_dev_mat=False, info=None):
        ...
        deps_contrib = self.deps_mat * repo_status
        repo_status_new = (
            abs(dev_contrib - deps_contrib) + dev_contrib + deps_contrib
        ) / 2
        repo_status_new = self.postprocess_repostatus(
            repo_status=repo_status_new, info=info
        )
...
class SRLinear(SRGetter):
    def propagate(self, dev_status, repo_status, force_dev_mat=False, info=None):
        ...
        dev_contrib = self.get_dev_contrib(dev_status=dev_status, force=force_dev_mat)
        deps_contrib = self.deps_mat * repo_status
        repo_status_new = (dev_contrib + deps_contrib) / 2.0
        repo_status_new = self.postprocess_repostatus(
            repo_status=repo_status_new, info=info
        )
        return repo_status_new
...
class OldSRCobbDouglas(SRGetter):
    def propagate(self, dev_status, repo_status, force_dev_mat=False, info=None):
        ...
        repo_status_new = (
            dev_contrib + deps_contrib - dev_contrib.multiply(deps_contrib)
        )  # = 1- (1-dev_c)(1-deps_c); Cobb Douglas with both exponents equal to 1 but adapted to sparse mat
