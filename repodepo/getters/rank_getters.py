...
class RepoRankGetter(Getter):
    ...
    def get(self, db, orig_id_rank=False, **kwargs):
        ...
        if not orig_id_rank:
            ...
            return self.reorder(
                prev_direct=ans_direct,
                prev_indirect=ans_indirect,
                prev_values=ans_values,
                db=db,
                **kwargs
            )

    def reorder(self, prev_direct, prev_indirect, prev_values, db, **kwargs):
        orig_direct, orig_indirect, orig_values = RepoRankGetter(db=db).get(
            db=db, orig_id_rank=False, **kwargs
        )
...
class UserRankGetter(RepoRankGetter):
    ...
    def reorder(self, prev_direct, prev_indirect, prev_values, db, **kwargs):
        orig_direct, orig_indirect, orig_values = UserRankGetter(db=db).get(
            db=db, orig_id_rank=False, **kwargs
        )
        reord_values = orig_values.copy()
        for i, v in enumerate(prev_values):
...
class RepoDLCorrectionRank(RepoDLRank):
    ...
    def correct_dls(self, prevec, deps_mat=None):
        ...
        for seed_vec in self.get_list_seedvecs(deps_mat=deps_mat):
            concerned_packages = self.get_concerned_packages(
                deps_mat=deps_mat, init_vec=seed_vec
            )
            ...
            remaining_dls = (
                remaining_dls - concerned_packages.multiply(seed_vec) * remaining_dls
            )  ####
